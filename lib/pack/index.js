...
function sommaire(projet, pieces, echecs, ch, separes = 0, pdfs = 0) {
  const lignes = [
...
export async function zipper(projet, plan, settings, options, flux) {
  ...
  zip.append(sommaire(projet, pieces, echecs, ch, separes.length, pdfs), { name: "SOMMAIRE.txt" });
  await zip.finalize();
  await fini;
  return { pieces: pieces.map(({ octets, ...r }) => r), separes: separes.length, pdfs, echecs };
}
