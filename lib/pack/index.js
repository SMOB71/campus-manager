...
export const PIECES = [
  ...
  { cle: "pilotage", nom: "Classeur de pilotage", dossier: "4 - Travailler hors application", ext: "xlsx",
    quoi: "La chaîne rejouée en formules : on saisit un retard, tout se recalcule.", faire: (p, l, c, o) => classeurPilotage(p, l, c, o) },
...
export async function ordresSepares(projet, plan, ch, options = {}) {
  const ext = ch?.format === "pdf" ? "pdf" : "docx";
  ...
  for (const [i, s] of seances.entries()) {
    ...
    out.push({
      nom: `${nomFichier(projet.nom)}-ODJ-${num}-${s.date}.${ext}`,
      chemin: `5 - Gouvernance/Ordres du jour/${nomFichier(projet.nom)}-ODJ-${num}-${s.date}.${ext}`,
      octets, type: ext, libelle: `Ordre du jour — ${s.libelle} du ${s.date}`,
...
export async function zipper(projet, plan, settings, options, flux) {
  ...
  try {
    ...
    // En PDF aussi : tout le reste du pack l'est, et un ordre du jour s'ouvre
    // le matin de la séance sur n'importe quel appareil.
    for (const s of await ordresSepares(projet, vue(plan), charte(settings, { format: "pdf" }), options)) {
      zip.append(s.octets, { name: `6 - PDF/${s.chemin}` });
    }
  } catch (e) {
