...
export async function planDetaille(projet, plan, ch) {
  ...
  for (const l of lots(plan)) {
    ...
    e.push(b.H3(`${l}  (${ts.length} actions${j.length ? `, ${j.length} jalon${j.length > 1 ? "s" : ""}` : ""})`),
      b.grille(["Action", "Début", "Fin", "Resp.", "Marge (j)", "Crit."], [44, 11, 11, 16, 10, 8],
        ts.map((t) => [t.titre, courtFR(t.debut), courtFR(t.fin), ou(t.responsable, "—"),
          t.jalon ? "—" : String(t.margeTotale ?? ""), t.critique ? "oui" : ""]), { size: 16 }));
  }
