...
import { OPENING_LOTS, OPENING_DEPTS, OPENING_JALONS, OPENING_SOURCES, critereFinDe, deptOf, buildOpeningBudget, OPENING_DUREE_REF } from "../calc.js";
  ...
  return {
    risques: [
      ...
      { titre: "La première promotion ne peut pas être recrutée via Parcoursup",
        ...
        commentaire: "Conséquence directe du calendrier : le dossier suppose un UAI, qui suppose le SIRET, qui suppose le Go/No-Go — et l'UAI n'arrive qu'en décembre sur onze mois. À trancher tôt : l'objectif d'inscrits et le plan média n'ont pas la même forme avec ou sans Parcoursup." },
...
export function planOuverture(o, taches, { aujourdhui = new Date().toISOString().slice(0, 10), settings = {} } = {}) {
  ...
  const tachesPack = items
    ...
    .map((t, i) => {
      ...
      return {
        ...
        // La base légale voyage avec l'action jusque dans les documents : une
        // date commandée par l'instruction d'une mairie ne se discute pas comme
        // une estimation de chef de projet, encore faut-il que le lecteur sache
        // laquelle il a sous les yeux.
        loi: t.loi || null,
        // Ce qui permet de dire « terminé » sans discuter. Vide = à définir, et
        // la fiche action l'affiche comme tel plutôt que d'inventer.
        critereFin: critereFinDe(t),
        baseLegale: t.loi && OPENING_SOURCES[t.loi] ? OPENING_SOURCES[t.loi].texte : "",
        regleLegale: t.loi && OPENING_SOURCES[t.loi] ? OPENING_SOURCES[t.loi].regle : "",
  ...
  const sources = [...new Set(tachesPack.map((t) => t.loi).filter(Boolean))]
    .map((k) => ({ cle: k, ...OPENING_SOURCES[k], actions: tachesPack.filter((t) => t.loi === k)
      .map((t) => ({ titre: t.titre, fin: t.fin, lot: t.lot })) }))
    .sort((a, b) => String(a.actions[0]?.fin).localeCompare(String(b.actions[0]?.fin)));
