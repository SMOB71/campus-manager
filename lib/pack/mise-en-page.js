...
import { briquesPdf } from "./mise-en-page-pdf.js";

// Deux moteurs, une seule surface. Les générateurs ne savent pas lequel les
// rend : c'est ce qui permet de produire le MÊME document en Word et en PDF
// sans l'écrire deux fois, et sans passer par une conversion qui perd les
// fonds de tableau.
export function briques(ch, margeLat = 1000) {
  if (ch?.format === "pdf") return briquesPdf(ch, margeLat);
  return briquesWord(ch, margeLat);
}

function briquesWord(ch, margeLat = 1000) {
  ...
  // SAUT et ligne existent dans les deux moteurs : sans eux, les générateurs
  // appelaient `new Paragraph({ children: [new PageBreak()] })` en dur, ce qui
  // les clouait au format Word.
  const SAUT = () => new Paragraph({ children: [new PageBreak()] });
  const ligne = (cellules) => new TableRow({ children: cellules });
