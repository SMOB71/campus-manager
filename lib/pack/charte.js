...
// `format` voyage avec la charte parce qu'elle est le seul objet qui traverse
// TOUS les générateurs. C'est lui qui décide du moteur de rendu — Word ou PDF —
// sans qu'aucun générateur ait à le savoir.
export function charte(settings = {}, { format = "docx" } = {}) {
  const c = settings?.pack || {};
  const out = { ...CHARTE_DEFAUT, format: format === "pdf" ? "pdf" : "docx" };
