...
function slidesDelaisLegaux(a, projet) {
  if (!projet.sources?.length) return [];
  return a.tableaux({ eyebrow: "Délais légaux", titre: "Les dates qui ne se négocient pas",
    chapeau: "Posées par un texte ou par le calendrier d'un tiers. Ajouter des moyens ne les raccourcit pas." },
    ["Texte ou calendrier", "Ce qu'il impose", "1re échéance"], [26, 60, 14],
    projet.sources.map((s) => [coupe(s.texte, 58), coupe(s.regle, 190), courtFR(s.actions[0]?.fin)]),
    { hauteur: 0.62 });
}
