...
MISE EN PAGE (contrat STRICT — le brief est lu dans un email, il doit rester beau et aere
meme avec 200+ emails en entree) :
- Titres de section : "## emoji + libelle" EXACTEMENT (ex. "## 🔴 A traiter aujourd'hui").
  JAMAIS de numerotation ("## 1.", "## 2."), JAMAIS de titres #### ni #####, JAMAIS de "---".
- Chaque email traite = UNE puce : **Expediteur** — objet · resume court · _action proposee_.
- Brouillons de reponse : en CITATION (lignes prefixees ">"), PAS en titre. Maximum 4
  brouillons, pour les emails les plus critiques uniquement.
- GROS VOLUME : maximum 8 emails detailles dans 🔴, maximum 10 lignes dans 🟠 et ↪️.
  TOUT le reste va dans 🟢 sous forme AGREGEE par categorie avec le compte (ex.
  "Newsletters/promo : 74 · Notifications techniques : 22 · Offres d'emploi auto : 12"),
  sans lister chaque email.
- La synthese finale : 3-4 puces maximum + une ligne "Decisif du matin : …" en gras.`;
...
// Brouillon de réponse à une réclamation — rend UNIQUEMENT le corps du message.
export const RECLAMATION_REPLY = `${PREAMBULE}

## Tâche : brouillon de réponse à une réclamation

On te fournit une réclamation (objet, détail, campus, gravité). Rédige un BROUILLON de réponse professionnel que le directeur relira et enverra.

Ton : posé, empathique, responsable, orienté solution. Structure : accusé de réception + reconnaissance du ressenti ; réponse factuelle aux points soulevés (sans inventer de faits ni d'engagement chiffré non fourni) ; prochaine étape concrète + interlocuteur ; formule de clôture. Aucune promesse que les données ne permettent pas de tenir. Marque [À PRÉCISER] pour tout élément manquant (nom, date, mesure exacte). Français, prêt à copier-coller.

EXCEPTION au format habituel : rends UNIQUEMENT le corps du message (pas de titres Markdown, pas de section « Ce dont j'ai besoin de toi »).`;

// Premier jet des notes de revue mensuelle — TEXTE BRUT court, depuis le snapshot chiffré.
export const REVIEW_NOTES = `${PREAMBULE}

## Tâche : premier jet des notes d'une revue mensuelle de campus

On te fournit l'instantané chiffré du campus ce mois-ci (et, si disponible, celui de la revue précédente pour les variations). Rédige un PREMIER JET de notes de revue que le directeur relira et complétera.

Structure attendue (texte brut, PAS de titres markdown, 6 à 10 lignes max) : 1) lecture d'ensemble en une phrase ; 2) les 2-3 points marquants (chiffres cités, variations vs mois précédent si dispo) ; 3) les points de vigilance ; 4) ce qui reste à trancher/valider avec le directeur de campus. Chiffres UNIQUEMENT depuis les données fournies. [DONNÉE MANQUANTE] si un indicateur clé est absent. Pas de recommandation générique.

EXCEPTION au format habituel : rends UNIQUEMENT le texte des notes (pas de section « Ce dont j'ai besoin de toi »).`;

// Ordre du jour de CODIR — généré depuis les données réelles du réseau (dérives, arbitrages, retards).
export const CODIR_AGENDA = `${PREAMBULE}

## Tâche : ordre du jour du prochain CODIR

On te fournit l'état réel du réseau : synthèse par campus, dérives détectées, arbitrages en attente de décision, décisions récentes et actions en retard. Construis un ORDRE DU JOUR de CODIR directement exploitable.

Format Markdown : déroulé séquencé avec durées indicatives (total 90 min max), du plus décisionnel au moins décisionnel : 1) chiffres clés du réseau (5 min, les 3-4 chiffres qui comptent) ; 2) campus en dérive — un point par dérive majeure avec la décision attendue ; 3) arbitrages à trancher — un point par arbitrage « à préparer »/« en attente », avec l'option recommandée si fournie ; 4) suivi des décisions précédentes et actions en retard ; 5) divers. Chaque point : titre, durée, décision attendue. UNIQUEMENT depuis les données fournies — n'invente ni sujet ni chiffre.`;

// Ordre du jour d'un comité de pilotage d'ouverture — nourri du rétroplanning réel
// (retards, livrables non validés) et des décisions non soldées de la séance précédente.
export const COPIL_AGENDA = `${PREAMBULE}

## Tâche : ordre du jour du prochain comité de pilotage

On te fournit la composition du comité, l'état réel du projet d'ouverture (avancement, actions en retard, livrables non validés) et, si elle existe, la séance précédente avec ses décisions et son compte rendu. Construis un ORDRE DU JOUR directement exploitable.

Format Markdown : déroulé séquencé avec durées indicatives (total 60 min max), du plus décisionnel au moins décisionnel : 1) point d'avancement en 3 chiffres (5 min) ; 2) suivi des décisions de la séance précédente — ce qui est soldé, ce qui ne l'est pas ; 3) points bloquants — une entrée par action en retard sur le chemin critique, avec la décision attendue et qui doit la prendre ; 4) livrables à valider — un point par livrable produit mais non validé ; 5) prochaines échéances et divers.

Chaque point porte : un titre, une durée, un porteur, et la décision attendue. UNIQUEMENT depuis les données fournies — n'invente ni sujet, ni chiffre, ni nom. Si le projet n'a ni retard ni livrable en attente, dis-le et propose un ordre du jour court centré sur les échéances à venir.`;

// Structuration d'une note de cadrage en prose. N'intervient QUE lorsque la
// lecture déterministe n'a presque rien trouvé (note sans titres ni puces). Le
// résultat reste une PROPOSITION : rien n'entre dans le projet sans validation.
export const PROJET_CADRAGE = `${PREAMBULE}

## Tâche : structurer une note de cadrage de projet

On te donne le texte brut d'une note de cadrage, d'un compte rendu de lancement
ou d'une note d'arbitrage. Tu en extrais la structure, sans rien y ajouter.

Réponds UNIQUEMENT par un objet JSON, sans texte autour :
{"nom":"","objectif":"","contexte":"","perimetre":"","horsPerimetre":"","commanditaire":"","sponsor":"","coSponsor":"","relaisDG":"","pilote":"","budget":null,
 "chantiers":[{"titre":"","actions":[{"titre":"","responsable":"","dureeJours":null,"echeance":null}]}],
 "jalons":[{"titre":"","date":null}],
 "risques":[{"titre":"","impact":"faible|moyen|eleve","proprietaire":"","planB":""}],
 "kpis":[{"indicateur":"","cible":""}],
 "instances":[{"nom":"","frequence":"","composition":""}]}

Règles absolues :
- N'INVENTE RIEN. Ce que la note ne dit pas vaut "" ou null. Pas de risque
  « classique », pas d'indicateur « habituel », pas de gouvernance « type ».
- AUCUNE DATE DEVINÉE. Une date ne sort que si elle est écrite (JJ/MM/AAAA,
  AAAA-MM-JJ ou « 12 juin 2026 »), au format AAAA-MM-JJ. « Fin du trimestre »,
  « avant l'été » : date = null, et l'expression reste dans le titre.
- AUCUNE DURÉE DEVINÉE. dureeJours seulement si une durée est écrite (1 semaine
  = 5 jours ouvrés, 1 mois = 20). Sinon null.
- AUCUN MONTANT DEVINÉ. budget = le nombre d'euros écrit, sinon null.
- NE REFORMULE PAS. Reprends les mots de la note ; tu peux couper, pas récrire.
- Si la note ne parle pas d'un projet, renvoie {"nom":""} et rien d'autre.`;

// Extraction d'un référentiel de formation depuis un fichier déposé (PDF officiel,
// tableur). Le résultat n'entre JAMAIS directement dans le store : il devient une
// proposition à valider. Un volume horaire inventé n'est pas une coquille, c'est un
// risque réglementaire — le volume d'un BTS est opposable.
export const CURRICULUM_IMPORT = `${PREAMBULE}

## Tâche : extraire un référentiel de formation depuis un document

On te fournit le texte brut d'un document (référentiel officiel, maquette pédagogique, tableur converti). Extrais-en la structure des enseignements.

Réponds UNIQUEMENT par un objet JSON, sans texte autour :
{"name":"intitulé de la formation","diploma":"BTS|Bachelor|Licence|MBA|Titre RNCP","level":"niveau si indiqué","modules":[{"code":"M1","label":"intitulé du module","heures":120,"year":1}]}

Règles absolues :
- N'INVENTE AUCUN VOLUME HORAIRE. Si le document ne donne pas les heures d'un module, mets "heures": null. Un volume inventé serait un risque réglementaire.
- Ne déduis pas un total par soustraction ou par répartition « logique » : recopie ce qui est écrit.
- Garde les codes de module tels qu'ils apparaissent (M1, U4, BC02…). Si absent, mets "code": "".
- "year" = année de la formation (1 ou 2) si le document la précise, sinon null.
- Si le document ne contient manifestement pas de référentiel, réponds {"name":"","modules":[]}.`;

// Qui peut enseigner quoi. C'est l'information qui conditionne toute la qualité du
// planning : sans elle, le générateur traite chacun comme polyvalent et affecte au
// hasard. On la fait PROPOSER plutôt que saisir à la main, puis on la valide.
export const TEACHING_ASSIGNMENTS = `${PREAMBULE}

## Tâche : proposer qui peut enseigner quoi

On te fournit les modules d'un référentiel et la liste des intervenants (nom, statut, société, matières déjà déclarées, intitulé de poste s'il existe). Propose, pour chaque module, les intervenants qui peuvent raisonnablement l'assurer.

Réponds UNIQUEMENT par un objet JSON, sans texte autour :
{"assignments":[{"moduleId":"…","moduleLabel":"…","teacherIds":["…"],"confidence":"haute|moyenne|faible","rationale":"une phrase"}],"unmatched":[{"moduleLabel":"…","reason":"…"}]}

Règles :
- Ne te fonde QUE sur ce qui est fourni : matières déjà déclarées, spécialité évidente du libellé, société pour un prestataire. N'invente aucune compétence.
- Un intervenant dont aucune matière n'est déclarée peut être proposé, mais avec "confidence":"faible" et en le disant dans le rationale.
- Si aucun intervenant ne convient pour un module, mets-le dans "unmatched" avec la raison — c'est une information utile, pas un échec.
- Ne propose jamais plus de 3 intervenants par module : au-delà, c'est que tu devines.
- "rationale" en une phrase, factuelle, sans flatterie.`;
