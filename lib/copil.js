...
// Même principe que pour une ouverture, sur un projet : le destinataire doit
// savoir où en est le sujet AVANT d'ouvrir le planning. Renvoie null pour tout
// autre périmètre — les comités campus et réseau ne changent pas d'un octet.
function contexteProjet(committee) {
  if (committee.scope !== "projet" || !committee.scopeId) return null;
  const p = store.getProjet(committee.scopeId);
  if (!p) return null;
  const plan = projets.ordonnancer(p, store.listProjetTaches({ projetId: p.id }), { aujourdhui: new Date().toISOString().slice(0, 10) });
  if (!plan.ok) return { projet: p, plan: null };
  return { projet: p, plan, derive: p.reference ? projets.derive(plan, p.reference) : null };
}

function htmlContexteProjet(ctx) {
  if (!ctx) return "";
  const { projet: p, plan, derive: d } = ctx;
  if (!plan) {
    return bloc("Où en est le projet", `<p style="font-size:14px;margin:0;color:#C94B33;">
      Le planning de « ${esc(p.nom)} » n'est pas calculable en l'état : aucune date n'est annoncée ici tant qu'il n'est pas repris.</p>`);
  }
  const r = plan.resume;
  const retard = d?.finEcart > 0;
  const etat = d
    ? (retard
      ? `<strong style="color:#C94B33;">${d.finEcart} jour${d.finEcart > 1 ? "s" : ""} de retard sur la référence.</strong>`
      : `<strong style="color:#0B6E5F;">Conforme à la référence figée.</strong>`)
    : `<span style="color:#7A8590;">Aucune référence figée : la dérive n'est pas mesurable.</span>`;
  const bloquants = r.echeancesDepassees.slice(0, 4);
  return bloc("Où en est le projet", `
    <p style="font-size:14px;margin:0 0 8px;">${esc(p.nom)} — fin prévue ${frDate(r.fin)} · avancement ${Math.round((r.avancement || 0) * 100)} % · reste ${r.resteAFaireJours} j. ${etat}</p>
    ${bloquants.length ? `<table style="border-collapse:collapse;width:100%;font-size:13px;">
      <thead><tr style="background:#E4F1EE;color:#0B6E5F;text-align:left;">
        <th style="padding:6px 9px;">Échéance dépassée</th><th style="padding:6px 9px;">Retard</th></tr></thead>
      <tbody>${bloquants.map((e) => `<tr>
        <td style="padding:6px 9px;border-top:1px solid #EDE7DA;">${esc(e.titre)}</td>
        <td style="padding:6px 9px;border-top:1px solid #EDE7DA;color:#C94B33;">${e.retard} j</td></tr>`).join("")}</tbody></table>` : ""}`);
}
...
export function convocationHtml(committee, session, { rappel = false } = {}) {
  ...
  return SHELL(esc(titre), `
    ...
    ${htmlContexte(ctx)}
    ${htmlContexteProjet(contexteProjet(committee))}
...
export async function runSessionReminders(today = new Date().toISOString().slice(0, 10), { envoyer = sendMail } = {}) {
  const out = { convocation: 0, rappel: 0, "compte-rendu": 0, relances: 0, erreurs: [] };
  for (const c of store.listCommittees()) {
    for (const s of c.sessions || []) {
      ...
      // Compte rendu ÉCRIT : on le diffuse, le lendemain de la séance au plus tôt — le
      // délai laisse le temps de corriger un brouillon avant qu'il ne parte. Sans cette
      // diffusion, un compte rendu rédigé reste dans l'app et personne ne l'a lu.
      if (reste <= -1 && s.minutes?.trim() && c.autoSendMinutes !== false && !s.sent?.["compte-rendu"]) {
        try {
          const res = await sendSessionMail(c.id, s.id, "compte-rendu", { envoyer });
          if (res.sent) out["compte-rendu"]++;
        } catch (e) { out.erreurs.push(`${c.name} compte-rendu : ${e?.message || e}`); }
      }
      // Écrit mais NON diffusé (diffusion automatique désactivée) : on relance. Un compte
      // rendu qui dort dans l'outil n'a pas plus de valeur qu'un compte rendu absent.
      if (reste <= -4 && s.minutes?.trim() && !s.sent?.["compte-rendu"] && !s.sent?.["relance-diffusion"]) {
        const d = destinataires(c);
        if (d.to.length && store.markSessionSent(c.id, s.id, "relance-diffusion")) {
          try {
            await envoyer({
              to: d.to.join(","),
              subject: `Compte rendu rédigé mais non diffusé — ${c.name} — ${frDate(s.date)}`,
              html: SHELL("Compte rendu non diffusé", `<p style="font-size:14px;">Le compte rendu de la séance du <strong>${frDate(s.date)}</strong> de « ${esc(c.name)} » est rédigé mais n'a été envoyé à personne.</p>
                <p style="font-size:14px;">Ouvre la séance dans Campus Manager pour le diffuser.</p>`),
            });
            out.relances++;
          } catch (e) { out.erreurs.push(`${c.name} relance-diffusion : ${e?.message || e}`); }
        }
      }
