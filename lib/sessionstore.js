...
// Même mécanique que le magasin principal, et pour la même raison : le fichier
// était réécrit EN ENTIER à chaque séance créée. Un réseau de 30 campus produit
// plus de 300 000 séances par an, et le planning est le magasin le PLUS écrit de
// l'application — un emploi du temps hebdomadaire en crée des centaines d'un
// coup. Le banc d'essai n'atteignait pas 30 000 séances en dix minutes.
const _moteur = creerMoteur({ collections: ["sessions"], nom: "planning" });
...
function write(db) {
  if (usePg()) {
    // Le cache reste l'état vivant : on ne l'invalide pas. Le relire depuis le
    // disque à chaque écriture était l'autre moitié du coût.
...
function normSession(s = {}) {
  return {
    ...
    // Présentiel, distance ou hybride. Sans ce champ, rien ne distingue une
    // heure signée sur une feuille d'une heure à distance, qui ne se prouve
    // pas de la même façon.
    modalite: ["presentiel", "distanciel", "hybride"].includes(s.modalite) ? s.modalite : "presentiel",
...
export function updateSession(sid, patch = {}) {
  ...
  // Sans cette ligne, passer une séance à distance depuis la fiche partait sans
  // erreur et ne changeait rien : le suivi à distance serait resté vide.
  if ("modalite" in patch && ["presentiel", "distanciel", "hybride"].includes(patch.modalite)) s.modalite = patch.modalite;
