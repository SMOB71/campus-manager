...
export const COLLECTIONS = {
  ...
  apiKeys: [],
  campagnes: ["campusId"],
  envois: ["campagneId", "destinataireId"],
  signatures: ["campusId"],
  mobilites: ["learnerId"],
  taxeCampagnes: ["campusId"],
  indicateursDeclares: ["campusId"],
  // Enquêtes. Les réponses ne portent AUCUN identifiant de répondant : la
  // colonne générée est l'enquête, jamais l'apprenant. Indexer ici un
  // learnerId rétablirait en base le lien que la soumission a rompu.
  enquetes: ["campusId"],
  enqueteInvitations: ["enqueteId", "destinataireId"],
  enqueteReponses: ["enqueteId"],
  // Indicateur 16 : une certification par référentiel, avec son habilitation
  // et ses sessions. Indicateur 29 : le registre des actions d'insertion.
  certifications: ["campusId", "curriculumId"],
  actionsInsertion: ["campusId"],
  // LMS. `lmsTraces` porte des consultations et des travaux rendus : indexé par
  // ressource et par séance, jamais par apprenant seul — on ne veut pas offrir
  // une requête « tout ce qu'untel a consulté » qui ferait un profil de navigation.
  ressources: ["campusId", "moduleId"],
  lmsTraces: ["ressourceId", "seanceId"],
  // Actes de formation (convention / contrat) et remises du règlement intérieur.
  actes: ["campusId", "classId"],
  remisesReglement: ["campusId", "learnerId"],
  // Contrats des intervenants et attestations de vigilance URSSAF.
  contratsIntervenants: ["campusId", "teacherId"],
  attestationsVigilance: ["contratId"],
  // Catalogue et devis : la chaîne offre -> devis -> acte -> facturation.
  offres: ["campusId"],
  devis: ["campusId", "offreId"],
  // Journal d'exploitation : la trace des exports, jamais leur contenu.
  journalExports: ["campusId"],
  // Répertoire projet : une seule liste alimente les membres de comité ET les
  // responsabilités des étapes. Deux listes parallèles divergeraient.
  personnes: ["campusId"],
  // Dossiers de financement individuel (CPF).
  dossiersCpf: ["campusId", "offreId", "learnerId"],
  // Planning. Les colonnes générées reprennent exactement les filtres de
  // listSessions() : campus, classe, professeur, salle, date, série.
  sessions: ["campusId", "classId", "teacherId", "roomId", "date", "seriesId"],
  // Conduite de projet. Les tâches sont une collection À PART et non un tableau
  // dans le projet : un projet d'ouverture porte deux à trois cents tâches que
  // l'on modifie une par une, et réécrire le document entier à chaque case
  // cochée ramènerait le problème que la bascule en base a résolu.
  projets: ["campusId"],
  projetTaches: ["projetId"],
  projetModeles: [],
  projetJournal: ["projetId"],
};
...
export function getPool() {
  if (!pool) {
    ...
    // Le schéma est posé PAR LE SERVEUR à l'ouverture de la connexion, via le
    // paramètre `options`. Le faire avec un `set search_path` dans le hook
    // « connect » était une course : node-postgres n'attend pas cette requête
    // avant de laisser passer la suivante, qui pouvait donc viser le mauvais
    // schéma. Le pilote émettait même un avertissement — il avait raison.
    const schema = process.env.DB_SCHEMA;
    if (schema && !/^[a-z_][a-z0-9_]{0,62}$/.test(schema)) throw new Error(`DB_SCHEMA invalide : ${schema}`);
