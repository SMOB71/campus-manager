...
// Sanitizer HTML côté serveur (même défense que le SPA). Le contenu des livrables est
// généré par le modèle et peut recopier du HTML actif issu d'un email/document piégé
// (<img onerror>, <script>) → on nettoie AVANT de servir l'export HTML/PDF.
const DOMPurify = createDOMPurify(new JSDOM("").window);
// Échappement contexte texte (titre, meta) — injectés hors markdown.
...
function printHtml(deliverable) {
  const body = DOMPurify.sanitize(marked.parse(stripKpis(deliverable.content)));
  ...
  return `<!doctype html><html lang="fr"><head><meta charset="utf-8"><title>${esc(deliverable.title)}</title>
<style>
  :root{
...
<body>
  ...
  <div class="sheet">
    ...
    <h1 class="title">${esc(deliverable.title)}</h1>
