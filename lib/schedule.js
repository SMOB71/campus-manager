...
export function conflictsFor(session, others = [], ctx = {}) {
  ...
  for (const p of periodsOn(session.date, ctx.periods, session.classId)) {
    if (p.kind === "vacances" || p.kind === "ferie") {
      ...
      push("block-forcable", "calendrier", `La classe est en stage du ${p.from} au ${p.to}.`);
    }
  }
...
export function coverage(curriculum, sessions = [], { today, endDate } = {}) {
  const mods = curriculum?.modules || [];
  ...
  const rows = mods.map((m) => {
    ...
    const due = m.heures == null ? 0 : Number(m.heures) || 0;
...
export function expandWeekly(base, untilISO, periods = []) {
  ...
  const closed = (periods || []).filter((p) => p && p.from && p.to
    && ["vacances", "ferie", "stage"].includes(p.kind)
