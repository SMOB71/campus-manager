...
import { controler as controlerDurees } from "./dureelegale.js";
import { couvre as couvreContrat } from "./contratsintervenants.js";
...
export function conflictsFor(session, others = [], ctx = {}) {
  ...
  // MOBILITÉ INTERNATIONALE — les mobilités étaient déjà neutralisées dans les
  // calculs d'assiduité (lib/mobilite.js), mais le planning ne les voyait pas :
  // rien n'empêchait de poser un cours pendant qu'un apprenti est en Espagne.
  // L'émargement le rattrapait après coup, jamais avant.
  //
  // On ne bloque pas sur un seul partant — une classe de vingt ne s'arrête pas
  // parce qu'un apprenti est en mobilité, et le signaler à chaque séance noierait
  // l'information. Ce qui compte, c'est la part du groupe concernée.
  const enMobilite = (ctx.mobilites || []).filter((m) => m?.from && m?.to
    && session.date >= m.from && session.date <= m.to);
  if (enMobilite.length) {
    const inscrits = ctx.effectifClasse || 0;
    const part = inscrits ? enMobilite.length / inscrits : 1;
    const ou = [...new Set(enMobilite.map((m) => m.pays).filter(Boolean))].join(", ");
    if (part >= 0.5) {
      push("block-forcable", "mobilite",
        `${enMobilite.length} inscrit(s) sur ${inscrits || "?"} sont en mobilité${ou ? ` (${ou})` : ""} à cette date : le cours se tiendrait devant une classe absente.`);
    } else {
      push("warn", "mobilite",
        `${enMobilite.length} inscrit(s) en mobilité${ou ? ` (${ou})` : ""} à cette date — le contrat d'apprentissage est mis en veille (art. L. 6222-42), leur absence n'est pas imputable.`);
    }
  }

  // COUVERTURE CONTRACTUELLE DE L'INTERVENANT. Affecter une séance à quelqu'un
  // dont aucun contrat ne couvre la date, c'est le faire travailler sans support
  // contractuel — et sur un CDD, c'est la poursuite au-delà du terme qui
  // requalifie en CDI sans que personne n'ait rien signé (L. 1243-11).
  //
  // Forçable, et non bloquant : on planifie en juillet ce qu'on contractualise
  // en août, et refuser sèchement rendrait le planning inutilisable. Mais le
  // forçage est tracé, et c'est ce qui permet de le retrouver.
  if (ctx.teacher && Array.isArray(ctx.contratsIntervenant)) {
    if (!ctx.contratsIntervenant.length) {
      // Simple ALERTE : « aucun contrat enregistré » dit surtout que le module
      // n'est pas encore alimenté — l'organisme a peut-être des contrats papier.
      // En faire un blocage forçable punirait la non-adoption et obligerait à
      // confirmer chaque séance de chaque intervenant, ce qui ferait ignorer le
      // message en une matinée.
      push("warn", "contrat",
        `${ctx.teacher.name || "Cet intervenant"} n'a aucun contrat enregistré : rien ne permet de vérifier que cette séance est couverte.`);
    } else if (!ctx.contratsIntervenant.some((c) => couvreContrat(c, session.date))) {
      // Là, en revanche, le module EST alimenté et la date tombe hors de toute
      // couverture : le constat est spécifique, il mérite une confirmation.
      push("block-forcable", "contrat",
        `Aucun contrat de ${ctx.teacher.name || "cet intervenant"} ne couvre le ${session.date}. Sur un contrat à durée déterminée, poursuivre au-delà du terme le transforme en contrat à durée indéterminée (art. L. 1243-11).`);
    }
  }
  ...
  // Le temps en centre de formation est du TEMPS DE TRAVAIL EFFECTIF : les
  // durées maximales, les repos et les pauses s'y appliquent. Ce contrôle vient
  // en dernier parce qu'il a besoin de la séance envisagée ET des autres séances
  // du groupe : une durée quotidienne ne se juge pas sur une séance isolée.
  //
  // Ses blocages sont volontairement « block » et non « block-forcable » : sur
  // une limite d'ordre public, un forçage serait une invitation à l'infraction.
  // Le seul passage prévu est une dérogation enregistrée.
  if (ctx.naissances?.length || ctx.naissanceIntervenant) {
    const memeGroupe = (o) => (session.classId ? o.classId === session.classId : o.teacherId === session.teacherId);
    const seances = [session, ...others.filter((o) => o.id !== session.id && o.status !== "cancelled" && memeGroupe(o))];
    const d = controlerDurees({
      seances,
      naissances: ctx.naissanceIntervenant ? [ctx.naissanceIntervenant] : ctx.naissances,
      dateRef: session.date,
      derogation: ctx.derogation || null,
      contexte: ctx.naissanceIntervenant ? "intervenant" : "groupe",
    });
    for (const v of d.violations) push(v.level, `duree-${v.code}`, `${v.message} (art. ${v.article})`);
    if (d.reserve) push("info", "duree-reserve", d.reserve);
  }

  const rank = { block: 0, "block-forcable": 1, warn: 2, info: 3 };
  return out.sort((a, b) => (rank[a.level] ?? 9) - (rank[b.level] ?? 9));
}
