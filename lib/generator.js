...
export function buildDemand(classes, curricula, opts = {}) {
  ...
  for (const k of classes) {
    ...
    for (const m of cur.modules || []) {
      // Un référentiel décrit TOUT le diplôme, sur ses deux années. Une promotion de
      // 1re année ne suit pas les modules de 2e : sans ce filtre, on lui demandait
      // 32 h hebdomadaires au lieu de 18, et le générateur échouait pour cause de
      // journées pleines — un diagnostic juste, sur une demande fausse.
      if (m.year != null && k.year != null && Number(m.year) !== Number(k.year)) continue;
      // Une classe en alternance n'a qu'une fraction des semaines de cours.
      const weeks = k.weeksAtSchool || (k.modalite === "alternance" ? Math.round(o.weeksInYear / 2) : o.weeksInYear);
      // L'hebdomadaire de la maquette prime : c'est la donnée officielle, saisie telle
      // quelle. Le volume annuel n'est qu'un repli pour les formations décrites ainsi.
      const total = m.heures == null ? 0 : Number(m.heures) || 0;
      let weekly = m.heuresSemaine != null ? Number(m.heuresSemaine) : null;
      if (weekly == null) {
        if (total <= 0) continue;
        weekly = total / weeks;
      }
      ...
      for (let i = 0; i < perWeek; i++) {
        units.push({
          ...
          weeks,                       // semaines de cours de CETTE classe
          annualHours: total,
        });
      }
    }
...
// `openingHours` (horaires réels du campus, jour par jour) prime sur dayStart/dayEnd :
// un campus fermé le mercredi après-midi ne doit pas voir de créneaux y apparaître.
export function buildSlots(opts = {}, openingHours = null) {
  const o = { ...DEFAULT_OPTIONS, ...opts };
  const lunchA = toMin(o.lunchStart), lunchB = toMin(o.lunchEnd);
  const slots = [];
  const days = openingHours ? o.days.filter((d) => (openingHours[d] || []).length) : o.days;
  for (const day of days) {
    const ranges = openingHours?.[day]?.length ? openingHours[day] : [[o.dayStart, o.dayEnd]];
    for (const [rs, re] of ranges) {
    const start = toMin(rs), end = toMin(re);
...
function newState() {
  return { placed: [], busy: { teacher: new Map(), room: new Map(), klass: new Map() }, hours: { teacher: new Map(), klass: new Map(), teacherYear: new Map() } };
}
...
export function whyNot(unit, slot, teacher, room, state, ctx, o) {
  ...
  if (teacher) {
    ...
    if (o.enforceLegalLimits) {
      ...
      if (due != null && due > 0) {
        // Chaque séance vaut les semaines de SA classe : annualiser tout le monde sur
        // 36 semaines surestimait du double un intervenant qui ne fait que de
        // l'alternance (17 semaines), et déclenchait un faux dépassement.
        const annual = (state.hours.teacherYear.get(teacher.id) || 0) + hours * (unit.weeks || o.weeksInYear);
...
function apply(state, unit, choice) {
  ...
  if (teacher) {
    addHours(state.hours.teacher, `${teacher.id}|${slot.day}`, h);
    addHours(state.hours.teacher, teacher.id, h);
    addHours(state.hours.teacherYear, teacher.id, h * (unit.weeks || 36));
  }
...
export function generateWeek(ctx, options = {}) {
  ...
  const slots = buildSlots(o, ctx.openingHours || null);
...
export function diagnose(unplaced, ctx) {
  ...
  // La raison la plus FRÉQUENTE n'est pas la plus INFORMATIVE : « pas d'équipement »
  // compté sur quatre salles jamais candidates masque « atelier trop petit » sur la
  // seule qui convenait. On classe donc par pouvoir explicatif décroissant.
  const PRIORITE = ["salle-trop-petite", "prof-plafond-hsupp", "prof-plafond-hebdo",
    "aucune-salle", "aucun-prof-declare", "salle-specifique-absente", "prof-indisponible",
    "prof-journee-pleine", "classe-journee-pleine", "salle-occupee", "prof-occupe", "classe-occupee"];
  const meilleure = (raisons) => {
    const codes = (raisons || []).map((r) => r[0]);
    for (const p of PRIORITE) if (codes.includes(p)) return p;
    return codes[0];
  };
  const byReason = new Map();
  for (const u of unplaced) {
    const top = meilleure(u.reasons);
    if (!top) continue;
