...
// Adresse publique de l'application. Elle etait ecrite en dur — et sur l'ANCIEN domaine :
// chaque alerte quotidienne et hebdomadaire renvoyait les destinataires vers un site qui
// n'est plus le bon. Un lien mort dans un email d'alerte, c'est l'alerte qu'on cesse
// d'ouvrir.
const APP_URL = (process.env.APP_URL || "https://campusmanager.fr").replace(/\/+$/, "");
const APP_HOTE = APP_URL.replace(/^https?:\/\//, "");
...
export function buildDigestHtml({ overdue, soon }, horizonDays, openings = []) {
...
<body style="margin:0;background:#F4EFE6;padding:24px;font-family:-apple-system,Segoe UI,Roboto,Arial,sans-serif;color:#0D1B2A;">
  <div style="max-width:720px;margin:0 auto;background:#fff;border:1px solid #E2DACD;border-radius:14px;overflow:hidden;">
    ...
    <div style="padding:12px 22px;color:#5A6672;font-size:12px;border-top:1px solid #E2DACD;">
      Suivi complet sur <a href="${APP_URL}" style="color:#0B6E5F;">${APP_HOTE}</a> → Plans d'action.
    </div>
  </div>
</body></html>`;
}
...
export function collectOpeningAlerts() {
  ...
  for (const o of store.listOpenings()) {
    ...
    // « 7 tâches en retard » ne dit pas s'il faut s'inquiéter : certaines ont des semaines
    // de marge. Ce qui décide, c'est le glissement de la rentrée et LA tâche qui le cause.
    const ch = o.targetDate ? analyseChain(o.tasks || [], { targetDate: o.targetDate, today }) : null;
    out.push({ name: o.name, targetDate: o.targetDate, late, slip: ch?.slip || 0, top: ch?.ruptures?.[0] || null });
  }
  return out;
}
function openingsHtml(items) {
  ...
  return `<h3 style="color:#0B6E5F;margin:22px 0 8px;">Ouvertures — tâches en retard</h3>` + items.map((o) => `<div style="padding:8px 0;border-bottom:1px solid #EDE7DA;">
    ...
    ${o.slip > 0
      ? `<div style="margin-top:4px;font-size:13px;color:#C94B33;font-weight:700;">Ces retards repoussent la rentrée de ${o.slip} jour${o.slip > 1 ? "s" : ""}.${o.top ? ` En cause : ${esc(o.top.title)} (${o.top.ownDelay} j de retard, ${o.top.downstream} tâche${o.top.downstream > 1 ? "s" : ""} en aval).` : ""}</div>`
      : `<div style="margin-top:4px;font-size:13px;color:#7A8590;">Absorbé par les marges — la rentrée n'est pas menacée à ce stade.</div>`}
...
<body style="margin:0;background:#F4EFE6;padding:24px;font-family:-apple-system,Segoe UI,Roboto,Arial,sans-serif;color:#0D1B2A;">
  <div style="max-width:720px;margin:0 auto;background:#fff;border:1px solid #E2DACD;border-radius:14px;overflow:hidden;">
    ...
    <div style="padding:12px 22px;color:#5A6672;font-size:12px;border-top:1px solid #E2DACD;">Détail sur <a href="${APP_URL}" style="color:#0B6E5F;">${APP_HOTE}</a> → Réseau.</div>
  </div>
</body></html>`;
...
export async function sendDeadlineAlert(cfg) {
  ...
  // Les tâches d'ouverture n'apparaissaient QUE dans le digest du lundi : une tâche
  // critique due demain restait invisible six jours sur sept, alors que c'est justement
  // sur une ouverture que le retard coûte le plus cher.
  const openings = collectOpeningAlerts();
  const retardsOuv = openings.reduce((n, o) => n + o.late.length, 0);
  const total = data.overdue.length + data.soon.length + retardsOuv;
  if (!total && !cfg.alwaysSend) return { sent: false, count: 0 };
  const glissement = openings.reduce((m, o) => Math.max(m, o.slip || 0), 0);
  await sendMail({
    to: cfg.to,
    subject: `Campus Manager — ${data.overdue.length} en retard, ${data.soon.length} à venir`
      + (glissement > 0 ? ` · ouverture +${glissement} j` : ""),
    // Passé en paramètre, pas injecté par remplacement de chaîne : l'ancre que j'avais
    // choisie n'existait pas dans le gabarit, et la section serait tombée dans le vide
    // sans que rien ne le signale.
    html: buildDigestHtml(data, cfg.horizonDays, openings),
  });
  return { sent: true, count: total, overdue: data.overdue.length, soon: data.soon.length, ouvertures: retardsOuv, glissement };
}
