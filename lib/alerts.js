...
export function collectNetworkSignals() {
  ...
  for (const c of campuses) {
    ...
    const parts = [];
    if (occ != null) parts.push([0.30, Math.min(100, occ)]);
    if (qual != null) parts.push([0.30, qual]);
    parts.push([0.15, overdue > 0 ? Math.max(0, 100 - overdue * 20) : 100]);
    parts.push([0.10, visitDue ? 50 : 100]);
    if (k.satisfaction != null) parts.push([0.10, Math.max(0, Math.min(100, k.satisfaction * 10))]);
    parts.push([0.05, Math.max(0, 100 - openInc * 15)]);
