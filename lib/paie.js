...
//
//   • Un intervenant SALARIÉ (permanent, vacataire, intervenant) est payé par la
//     paie. Ses heures alimentent un bulletin de salaire.
//   • Un PRESTATAIRE (auto-entrepreneur, société) FACTURE. Ses heures ne doivent
//     JAMAIS entrer dans un export de paie.
//
// Les mélanger n'est pas une maladresse d'export : faire figurer un prestataire
// dans la paie, c'est produire la pièce qui servira à démontrer un lien de
// subordination et à requalifier le contrat. Le module refuse donc de le sortir
// dans un export de paie, et il dit pourquoi.
//
...
// On s'aligne sur les statuts DÉJÀ portés par le modèle intervenant
// (TEACHER_STATUS) plutôt que d'en créer un second jeu : deux vocabulaires pour
// la même réalité finissent toujours par diverger, et c'est l'utilisateur qui
// paie la confusion. Le « prestataire » est celui qui facture.
export const STATUTS = {
  permanent: { label: "Permanent", paie: true },
  vacataire: { label: "Vacataire", paie: true },
  intervenant: { label: "Intervenant salarié", paie: true },
  prestataire: { label: "Prestataire (facture)", paie: false },
};
...
export function validateIntervenant(t = {}) {
  ...
  if (t.status && !STATUTS[t.status]) errors.push(`statut inconnu (${Object.keys(STATUTS).join(", ")})`);
  if (!t.status) warnings.push("statut non renseigné — impossible de savoir s'il relève de la paie ou de la facturation");
  if (STATUTS[t.status]?.paie) {
...
export function ligneDePaie(intervenant, seances, { from, to } = {}) {
  const statut = STATUTS[intervenant?.status] || null;
  ...
  if (!statut.paie) {
    return {
      ...
      motif: "prestataire : il facture. L'inscrire dans un export de paie fournirait une pièce à l'appui d'une requalification en contrat de travail.",
