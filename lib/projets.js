...
export function normaliserTache(t = {}) {
  ...
  return {
    ...
    // Provenance dans le modèle. C'est cette clé — et non le titre ni la date,
    // qui bougent — qui permet de rejouer un modèle corrigé sur un plan déjà
    // en cours sans détruire ce qu'on y a saisi.
    modeleRef: texte(t.modeleRef, 40),
...
export function validerPlan(taches) {
  ...
  for (const t of taches) {
    ...
    if (estSynthese && t.dureeJours > 0) {
      // AVERTISSEMENT, pas blocage. Ajouter une sous-tâche à une tâche
      // existante en fait une tâche de synthèse : sa durée saisie (1 jour par
      // défaut) devient caduque, et le calcul l'ignore déjà au profit de
      // l'enveloppe de ses enfants. La traiter comme une erreur rendait le plan
      // entier « non calculable » au premier regroupement — pour une donnée qui
      // n'était même pas utilisée.
      erreurs.push({ tacheId: t.id, message: `« ${nom} » regroupe d'autres tâches : sa durée est celle de ses enfants, la durée saisie est ignorée`, corrigible: true, avertissement: true });
    }
...
// Rejouer un modèle sur un projet DÉJÀ EN COURS.
//
// « Régénérer depuis le modèle » en remplaçant tout coûterait les responsables,
// les statuts, les restes à faire, les affectations et les notes. Personne ne
// paie ce prix : on ne corrige donc jamais le modèle, et il pourrit. C'est le
// contraire de ce qu'on attend d'une trame vivante.
//
// L'appariement se fait sur `modeleRef`, posé à la création justement pour
// ça — il survit au changement de titre et de durée. À défaut (plan monté à la
// main), on retombe sur le titre normalisé.
//
// DEUX RÈGLES DE PRUDENCE, et elles priment sur la fidélité au modèle :
//   · une tâche ajoutée à la main n'a jamais appartenu au modèle, elle reste ;
//   · une tâche retirée du modèle n'est supprimée QUE si personne n'y a touché.
//     Effacer un travail consigné serait pire qu'un plan un peu long.
const clefTitre = (t) => String(t?.titre || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9]+/g, " ").trim();
const touchee = (t) => t.statut !== "a_faire" || !!t.responsable || (t.affectations || []).length > 0
  || !!texte(t.note) || (t.budgetDepense || 0) > 0 || !!t.debutReel || !!t.finReelle;

export function fusionnerModele(taches = [], modeleTaches = [], { modeleId = null } = {}) {
  const existantes = taches.map(normaliserTache);
  const parRef = new Map(existantes.filter((t) => t.modeleRef).map((t) => [t.modeleRef, t]));
  const parTitre = new Map();
  for (const t of existantes) if (!parTitre.has(clefTitre(t))) parTitre.set(clefTitre(t), t);

  const appariees = new Map();   // ref du modèle -> tâche enregistrée
  const majs = [], ajouts = [];
  for (const m of modeleTaches) {
    const ref = m.ref || m.id;
    const cible = parRef.get(ref) || parTitre.get(clefTitre(m));
    if (cible && !appariees.has(ref) && ![...appariees.values()].includes(cible)) {
      appariees.set(ref, cible);
      majs.push({ id: cible.id, ref, titre: m.titre, ancienTitre: cible.titre });
    } else if (!cible) {
      ajouts.push({ ref, titre: m.titre });
    }
  }

  // L'identifiant conservé est celui de la tâche ENREGISTRÉE : d'autres objets
  // le référencent (liens, journal). Les dépendances du modèle sont donc
  // réécrites vers les identifiants qui survivent.
  const idPourRef = new Map();
  for (const [ref, t] of appariees) idPourRef.set(ref, t.id);

  const conservees = existantes.filter((t) => ![...appariees.values()].includes(t) && (!t.modeleRef || t.modeleId !== modeleId));
  const disparues = existantes.filter((t) => t.modeleRef && t.modeleId === modeleId
    && !modeleTaches.some((m) => (m.ref || m.id) === t.modeleRef));
  const supprimables = disparues.filter((t) => !touchee(t));
  const gardees = disparues.filter((t) => touchee(t));

  return {
    // Ce que le modèle APPORTE sur une tâche appariée : la forme du plan.
    // Ce que la tâche GARDE : tout ce qu'un humain y a mis.
    majs: majs.map(({ id, ref }) => {
      const m = modeleTaches.find((x) => (x.ref || x.id) === ref);
      const cible = existantes.find((x) => x.id === id);
      return {
        id, ref, ancienTitre: cible.titre, titre: m.titre,
        patch: {
          titre: m.titre, lot: m.lot || cible.lot, jalon: !!m.jalon,
          dureeJours: m.jalon ? 0 : (m.dureeJours ?? cible.dureeJours),
          contrainte: m.contrainte || cible.contrainte,
          modeleRef: ref, modeleId,
          // Le reste à faire suit la durée SEULEMENT si rien n'a commencé.
          ...(cible.statut === "a_faire" ? { resteAFaire: m.jalon ? 0 : (m.dureeJours ?? cible.dureeJours) } : {}),
        },
      };
    }),
    ajouts, supprimables, gardees, conservees: conservees.length,
    // Les liens du modèle, traduits vers les identifiants qui survivent. Les
    // références vers une tâche que l'utilisateur a supprimée et gardée
    // supprimée sont abandonnées, sinon le plan deviendrait incalculable.
    liens: modeleTaches.map((m) => {
      const ref = m.ref || m.id;
      const id = idPourRef.get(ref);
      if (!id) return null;
      return { id, liens: (m.liens || []).map((l) => ({ deId: idPourRef.get(l.ref || l.deId), type: l.type, decalage: l.decalage || 0 })).filter((l) => l.deId) };
    }).filter(Boolean),
    parents: modeleTaches.map((m) => {
      const ref = m.ref || m.id;
      const id = idPourRef.get(ref);
      const pref = m.parentRef || m.parentId;
      if (!id || !pref) return null;
      const pid = idPourRef.get(pref);
      return pid ? { id, parentId: pid } : null;
    }).filter(Boolean),
  };
}
