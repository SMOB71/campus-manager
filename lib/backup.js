...
export async function copierVersDistant(fichier, cfg = config()) {
  ...
  try {
    await execFileP("rsync", ["-az", "--timeout=60", "-e", ssh, fichier, `${d.utilisateur}@${d.hote}:${d.chemin.replace(/\/?$/, "/")}`],
      { timeout: 120000 });
    const purge = await purgerDistant(cfg, ssh);
    return { envoye: true, cible: `${d.utilisateur}@${d.hote}:${d.chemin}`, ...(purge ? { purgees: purge } : {}) };
  } catch (e) {
...
// Sans purge, le dossier distant grossit indéfiniment jusqu'à saturer le disque du
// serveur de sauvegarde — panne qui se déclare toujours au pire moment. On applique
// LA MÊME sélection étagée que localement : le nom de fichier porte la date, donc la
// logique déjà testée s'y applique telle quelle.
async function purgerDistant(cfg, ssh) {
  const d = cfg.distant;
  try {
    const { stdout } = await execFileP("ssh", [...ssh.split(" ").slice(1), `${d.utilisateur}@${d.hote}`,
      `ls -1 ${JSON.stringify(d.chemin)} 2>/dev/null | grep '\\.cmbak$' || true`], { timeout: 30000 });
    const noms = stdout.split("\n").map((x) => x.trim()).filter(Boolean);
    if (!noms.length) return 0;
    const archives = noms.map((n) => ({ nom: n, createdAt: (n.match(/campus-(.+)\.cmbak/) || [])[1]?.replace(/-/g, (c, i) => (i === 13 || i === 16 ? ":" : c)) || n }))
      .sort((a, b) => b.nom.localeCompare(a.nom));
    const { supprimer } = selectionRetention(archives, cfg);
    if (!supprimer.length) return 0;
    await execFileP("ssh", [...ssh.split(" ").slice(1), `${d.utilisateur}@${d.hote}`,
      `cd ${JSON.stringify(d.chemin)} && rm -f ${supprimer.map((n) => JSON.stringify(n)).join(" ")}`], { timeout: 30000 });
    return supprimer.length;
  } catch { return 0; }   // une purge qui échoue ne doit jamais faire échouer la sauvegarde
}
