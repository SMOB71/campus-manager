...
const EMPTY = { campuses: [], deliverables: [], actions: [], visits: [], kpi: [], briefs: [], incidents: [], documents: [], partners: [], networkObjectives: [], audit: [], scenarios: [], openings: [], emailPriority: [], emailMuted: [], decisions: [], reviews: [], events: [], recoveries: [], arbitrages: [], committees: [], si: [], teachers: [], curricula: [], rooms: [], classes: [], periods: [], financeProposals: {}, curriculumProposals: {}, settings: {} };
...
function normClass(k = {}) {
  return {
    ...
    year: num(k.year), size: num(k.size), createdAt: k.createdAt || now(),
  };
}
...
export function updateClass(kid, patch = {}) {
  ...
  for (const f of ["year", "size"]) if (f in patch) k[f] = num(patch[f]);
  write(db);
  return k;
}
...
export const PERIOD_KINDS = ["vacances", "ferie", "examens", "stage"];
