...
// Réinitialise les envois d'une séance — utilisé quand sa date change : une convocation
// déjà partie pour l'ancienne date doit pouvoir repartir pour la nouvelle.
export function resetSessionSent(cid, sid) {
  const db = read();
  const c = (db.committees || []).find((x) => x.id === cid);
  const s = c && (c.sessions || []).find((x) => x.id === sid);
  if (!s) return null;
