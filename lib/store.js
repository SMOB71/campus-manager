...
const EMPTY = { campuses: [], deliverables: [], actions: [], visits: [], kpi: [], briefs: [], incidents: [], documents: [], partners: [], networkObjectives: [], audit: [], scenarios: [], openings: [], emailPriority: [], emailMuted: [], decisions: [], reviews: [], events: [], recoveries: [], arbitrages: [], committees: [], si: [], learners: [], enrollments: [], candidates: [], contracts: [], teachers: [], curricula: [], rooms: [], classes: [], periods: [], financeProposals: {}, curriculumProposals: {}, settings: {} };
...
function normLearner(l = {}) {
  ...
  return {
    ...
    dateNaissance: s(l.dateNaissance), lieuNaissance: s(l.lieuNaissance),
...
// RGPD : la suppression retire le dossier ET ses inscriptions ; les documents GED
// rattachés sont délestés (learnerId=null) mais restent au campus (pièces comptables).
export function deleteLearner(lid) {
  ...
  for (const d of db.documents || []) if (d.learnerId === lid) d.learnerId = null;
  write(db);
}
...
function normContract(c = {}) {
  ...
  return {
    ...
    dateDebut: s(c.dateDebut), dateFin: s(c.dateFin), dateSignature: s(c.dateSignature),
    ...
    dateDepot: s(c.dateDepot), dateValidation: s(c.dateValidation),
