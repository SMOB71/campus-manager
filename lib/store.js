...
export function updateQualiopi(cid, patch = {}) {
  ...
  // `categories` est une liste (af/app/bc/vae) : la passer par QUALIOPI_META la
  // transformerait en chaîne et le calcul de périmètre ne verrait plus rien.
  if (Array.isArray(patch.categories)) {
    c.qualiopi.categories = patch.categories.filter((x) => ["af", "app", "bc", "vae"].includes(x));
  }
