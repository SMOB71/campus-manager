...
import fs from "fs";
import { normalizeExemption } from "./acquis.js";
import { addMonths } from "./qualiopi.js";
import path from "path";
import crypto from "crypto";
import { encrypt, decrypt, isEncrypted, encryptionEnabled } from "./crypto-store.js";
import * as pg from "./db.js";
import { creerMoteur } from "./persistance.js";

const DATA_DIR = process.env.DATA_DIR || path.join(process.cwd(), "data");
const DB_FILE = path.join(DATA_DIR, "db.json");

const EMPTY = { campuses: [], deliverables: [], actions: [], visits: [], kpi: [], briefs: [], incidents: [], documents: [], partners: [], networkObjectives: [], audit: [], scenarios: [], openings: [], emailPriority: [], emailMuted: [], decisions: [], reviews: [], events: [], recoveries: [], arbitrages: [], committees: [], si: [], learners: [], enrollments: [], candidates: [], contracts: [], assessments: [], grades: [], portalAccess: [], fundings: [], invoices: [], payments: [], attendanceAnchors: [], teachers: [], curricula: [], rooms: [], classes: [], periods: [], blockExemptions: [], positionnements: [], amenagements: [], suivis: [], reclamations: [], sousTraitants: [], jurySessions: [], apiKeys: [], campagnes: [], envois: [], signatures: [], mobilites: [], taxeCampagnes: [], indicateursDeclares: [], enquetes: [], enqueteInvitations: [], enqueteReponses: [], certifications: [], actionsInsertion: [], ressources: [], lmsTraces: [], actes: [], remisesReglement: [], contratsIntervenants: [], attestationsVigilance: [], offres: [], devis: [], journalExports: [], personnes: [], dossiersCpf: [], projets: [], projetTaches: [], projetModeles: [], projetJournal: [], financeProposals: {}, curriculumProposals: {}, settings: {} };
...
// ---------------------------------------------------------------------------
// PERSISTANCE : fichier JSON (par défaut) ou PostgreSQL (si DATABASE_URL).
//
// Le fichier réécrivait TOUT à chaque mutation — 159 ms à 120 000 notes, et ça
// n'arrêtait jamais de croître. C'est ce qui interdisait le multi-site.
//
// COMMENT ON SAIT CE QU'IL FAUT ÉCRIRE, SANS TOUCHER AUX 147 POINTS DE MUTATION
//
// Le cache mémoire EST l'état vivant : les fonctions le mutent puis appellent
// write(). Il suffit donc de comparer l'état à un reflet pris après la dernière
// écriture. Encore faut-il que la comparaison soit à la fois juste et bon marché,
// et les deux ne vont pas ensemble — d'où une classification, vérifiée par un
// test structurel pour qu'elle ne dérive pas en silence :
//
//   • Collections JAMAIS mutées en place (notes, journal d'audit, documents,
//     règlements, suivis…) : elles ne connaissent que push / filter /
//     remplacement, donc changer une ligne change SA RÉFÉRENCE. Une comparaison
//     de références suffit — quelques millisecondes sur 120 000 lignes.
//     Ce sont précisément les collections qui explosent avec les années.
//
//   • Collections mutées EN PLACE (apprenants, inscriptions, contrats,
//     factures, candidatures, évaluations — Object.assign ou affectation de
//     champ) : la référence ne bouge pas, il faut comparer les VALEURS. C'est
//     plus cher, mais ces collections restent en milliers de lignes, pas en
//     centaines de milliers. Le coût est donc borné, là où celui du fichier ne
//     l'était pas.
//
// Une collection oubliée dans la seconde liste perdrait des mises à jour : le
// test « classification » relit le code source et échoue si une collection
// mutée en place n'y figure pas.
// ---------------------------------------------------------------------------

// La mécanique de persistance est partagée avec le planning : voir
// lib/persistance.js, qui porte les trois subtilités à ne jamais oublier
// (modification sur place invisible, marquage qui doit porter la LIGNE, et
// écritures enchaînées).
const _moteur = creerMoteur({
  collections: pg.COLLECTION_NAMES.filter((c) => c !== "sessions"),
  singletons: pg.SINGLETONS,
  nom: "magasin principal",
});
const usePg = () => _moteur.actif();
export const touch = _moteur.touch;
export const flush = _moteur.flush;
export const verifierCoherence = () => _moteur.verifierCoherence(_cache);

export async function init() {
  return _moteur.init((charge) => {
    _cache = { ...EMPTY, ...charge };
    return _cache;
  });
}
...
function write(db) {
  if (usePg()) {
    // En base, le cache reste l'état vivant : on ne l'invalide pas (le relire
    // depuis le disque à chaque écriture était l'autre moitié du coût).
    _moteur.persister(db);
    return;
  }
  ensure();
...
// DANGER HISTORIQUE. Ces trois fonctions opèrent sur db.json. Depuis le passage à
// PostgreSQL, ce fichier est FIGÉ à la date de la bascule : l'app ne l'écrit plus.
//   • backupNow() copiait donc un instantané périmé en l'horodatant du jour ;
//   • listBackupsMeta() présentait ces copies comme l'état sauvegardé, ce qui est faux ;
//   • restoreBackup() passait l'objet ENTIER à write() → sous PostgreSQL, le diff voit
//     des références neuves partout et réécrit toute la base : un clic dans l'interface
//     écrasait la production avec un instantané périmé, et sans même mettre à jour le
//     cache mémoire — base et mémoire divergeaient dans la foulée.
// En base, la sauvegarde est le pg_dump nocturne (cron campus-pg-backup, alerte email en
// cas d'échec, restauration vérifiée). On refuse ici plutôt que de faire semblant.
export const sauvegardeFichierActive = () => !usePg();
const REFUS_PG = { error: "En base PostgreSQL, les sauvegardes sont assurées par le dump nocturne côté serveur (cron campus-pg-backup). La sauvegarde fichier est désactivée : elle porterait sur un db.json figé depuis la bascule." };

export function backupNow() {
  if (usePg()) return REFUS_PG;
  ensure();
  const bdir = path.join(DATA_DIR, "backups");
  if (!fs.existsSync(bdir)) fs.mkdirSync(bdir, { recursive: true });
  const stamp = new Date().toISOString().replace(/[:.]/g, "-");
...
// `uai` (n° UAI) et `siret` identifient le CFA au dépôt du contrat ;
// `numeroDeclaration` est le n° de déclaration d'activité (BPF, Qualiopi).
// `referentHandicap` manquait a cette liste alors que lib/demarrage.js le
// CONTROLE : le point « referent handicap non designe » ne pouvait donc jamais
// etre leve, sur aucun campus. Une alerte permanente qu'on ne peut pas eteindre
// n'alerte plus — elle apprend a ignorer l'ecran qui la porte.
const CAMPUS_FIELDS = ["name", "address", "email", "phone", "city", "region", "network",
  "siret", "uai", "numeroDeclaration", "dirigeant", "referentHandicap", "referentMobilite"];
...
export function updateCampus(cid, patch = {}) {
  ...
  // Dispositif de formation à distance : bloc objet, donc invisible aux deux
  // boucles ci-dessus, qui n'acceptent que des chaînes et des nombres. Sans
  // cette ligne le patch partait sans erreur et le référent pédagogique ne
  // s'enregistrait jamais.
  // Budget des heures d'enseignement et coefficient de charges patronales.
  // Bloc objet, donc invisible aux deux boucles ci-dessus — même piège que foad.
  // Règlement intérieur et livret d'accueil : blocs objets, donc invisibles aux
  // boucles chaînes/nombres ci-dessus.
  if (patch.reglement && typeof patch.reglement === "object") {
    const t = (v) => String(v || "").trim();
    c.reglement = {
      version: t(patch.reglement.version), dateApplication: t(patch.reglement.dateApplication),
      hygieneSecurite: t(patch.reglement.hygieneSecurite), disciplinaire: t(patch.reglement.disciplinaire),
      sanctions: t(patch.reglement.sanctions), procedure: t(patch.reglement.procedure),
      representation: t(patch.reglement.representation),
    };
  }
  if (patch.livret && typeof patch.livret === "object") {
    c.livret = { contenu: String(patch.livret.contenu || "").trim() };
  }
  if (patch.masse && typeof patch.masse === "object") {
    const n = (v) => (v === "" || v == null ? null : Number(v));
    c.masse = {
      budgetAnnuel: n(patch.masse.budgetAnnuel),
      // Déclaré, jamais deviné : il dépend du statut et de la convention.
      coefficientCharges: n(patch.masse.coefficientCharges),
      anneeDebut: String(patch.masse.anneeDebut || "").trim(),
      anneeFin: String(patch.masse.anneeFin || "").trim(),
    };
  }
  if (patch.foad && typeof patch.foad === "object") {
    c.foad = {
      referentPedagogique: String(patch.foad.referentPedagogique || "").trim(),
      modalitesAssistance: String(patch.foad.modalitesAssistance || "").trim(),
    };
  }
  touch("campuses", c);
  write(db);
...
export function getQualiopi(cid) {
  ...
  if (!c) return null;
  // Un enregistrement sans version a été saisi sous le référentiel 2019 : le
  // supposer à jour ferait lire 32 statuts comme s'ils portaient sur les 33
  // indicateurs renumérotés du référentiel 2026.
  const q = c.qualiopi || { indicators: {} };
  // `valideJusquau` est LU par lib/demarrage.js et lib/cpf.js, et n'etait ecrit
  // nulle part : l'alerte « certification expiree » ne se declenchait jamais, et
  // cote financement individuel AUCUNE offre n'etait jamais eligible. On le
  // DERIVE plutot que d'ajouter un champ de plus a remplir — un certificat est
  // valable trois ans a compter de l'audit, et `renewalDate` porte deja cette
  // date quand elle a ete calculee.
  const valideJusquau = q.renewalDate || (q.lastAudit ? addMonths(q.lastAudit, 36) : null) || null;
  return { ...q, version: q.version || "v2019", valideJusquau };
}
// Bascule vers le référentiel 2026. Remplace le tableau d'un bloc — un patch
// champ par champ laisserait cohabiter deux numérotations dans le même objet.
export function setQualiopi(cid, valeur) {
  const db = read();
  const c = db.campuses.find((x) => x.id === cid);
  if (!c) return null;
  c.qualiopi = valeur;
...
export function updateQualiopi(cid, patch = {}) {
  ...
  if (!c.qualiopi.version) c.qualiopi.version = "v2019";
...
function normTeacher(t = {}) {
  return {
    ...
    // Paie : le matricule rapproche la ligne du logiciel de paie, la règle dit
    // comment les heures de face-à-face deviennent des heures payées. Sans eux,
    // l'export sortait incomplet et le rapprochement se faisait à la main.
    matricule: String(t.matricule || "").trim(),
    regle: String(t.regle || "").trim(),
...
export function updateTeacher(tid, patch = {}) {
  ...
  if (!t) return null;
  // `matricule` et `regle` manquaient à cette liste alors que normTeacher les
  // définit : un intervenant créé sans eux ne pouvait plus JAMAIS les recevoir,
  // le patch partant sans erreur. Ce sont précisément les deux champs dont
  // l'export de paie a besoin — il sortait donc incomplet sans qu'on puisse
  // le corriger depuis l'interface.
  for (const f of ["name", "email", "phone", "company", "contractRef", "matricule", "regle"]) if (f in patch) t[f] = String(patch[f] || "").trim();
...
function normModule(m = {}) {
  return {
    ...
    // Bloc de compétences RNCP auquel la matière contribue.
    blocId: m.blocId || null,
...
function normCurriculum(c = {}) {
  return {
    ...
    // Codes de certification exigés au dépôt.
    codeRncp: String(c.codeRncp || "").trim(), codeDiplome: String(c.codeDiplome || "").trim(),
    intitulePrecis: String(c.intitulePrecis || "").trim(),
    dureeHeures: c.dureeHeures == null || c.dureeHeures === "" ? null : Number(c.dureeHeures),
    dureeCycleAnnees: c.dureeCycleAnnees == null || c.dureeCycleAnnees === "" ? null : Number(c.dureeCycleAnnees),
    modules: Array.isArray(c.modules) ? c.modules.filter((m) => m && (m.label || m.code)).map(normModule) : [],
    // Blocs de compétences du titre : ce sont EUX qui se valident, pas la moyenne.
    blocks: Array.isArray(c.blocks) ? c.blocks.filter((b) => b && (b.label || b.code)).map(normBlock) : [],
...
function normBlock(b = {}) {
  const n = (v) => (v === "" || v == null ? null : Number(v));
  return {
    id: b.id || id(), code: String(b.code || "").trim(), label: String(b.label || "").trim(),
    seuil: n(b.seuil), noteEliminatoire: n(b.noteEliminatoire),
    // Rattachement explicite des matières : un bloc sans matière ne se calcule pas.
    moduleIds: Array.isArray(b.moduleIds) ? b.moduleIds.filter(Boolean) : [],
  };
}
...
export function updateCurriculum(cid, patch = {}) {
  ...
  if (!c) return null;
  for (const f of ["name", "diploma", "level", "source", "codeRncp", "codeDiplome", "intitulePrecis"]) if (f in patch) c[f] = String(patch[f] || "").trim();
  for (const f of ["dureeHeures", "dureeCycleAnnees"]) if (f in patch) c[f] = patch[f] === "" || patch[f] == null ? null : Number(patch[f]);
  if ("blocks" in patch) c.blocks = Array.isArray(patch.blocks) ? patch.blocks.filter((b) => b && (b.label || b.code)).map(normBlock) : [];
...
function normPeriod(p = {}) {
  return {
    ...
    // Provenance. Régénérer un rythme d'alternance remplace les périodes qu'il
    // avait lui-même posées, et SEULEMENT celles-là : une semaine saisie à la
    // main (un salon professionnel, une semaine d'intégration) ne doit pas
    // disparaître parce qu'on a rejoué le rythme.
    source: p.source === "rythme" ? "rythme" : "manuel",
...
export function addOpening(data = {}) {
  ...
  // dureeMois : duree visee du projet, en mois avant la rentree. Le plan s'y recalibre
  // (lib/calc.js recalibrer) — seul l'amont se comprime, la chaine reglementaire ne bouge pas.
  // `== null` et pas `||` : une valeur saisie doit primer, y compris si elle est basse.
  const o = { id: id(), status: OPENING_STATUS.includes(data.status) ? data.status : "etude", tasks: [], budgetLines: [], campusId: null, budget: data.budget ? Number(data.budget) : null, targetDate: data.targetDate || "", dureeMois: data.dureeMois == null || data.dureeMois === "" ? null : Number(data.dureeMois), createdAt: now() };
...
export function updateOpening(oid, patch = {}) {
  ...
  if ("dureeMois" in patch) o.dureeMois = patch.dureeMois == null || patch.dureeMois === "" ? null : Number(patch.dureeMois);
  touch("openings", o);
  write(db);
  return o;
}
...
function normTask(t) {
  return {
    ...
    // Date de réalisation EFFECTIVE, posée au passage à « done ». Sans elle, une tâche
    // faite trois semaines en retard finissait « à l'heure » aux yeux du calcul de chaîne,
    // et ne poussait aucune de ses suivantes : le retard disparaissait en la cochant.
    doneAt: t.doneAt || null,
    // CE QUI A ETE FAIT, en toutes lettres. Une case cochee ne dit rien : six mois
    // plus tard, « conseil de perfectionnement : fait » n'apprend ni quand, ni avec
    // qui, ni ce qui s'y est decide. C'est le seul element qui survit au depart de
    // celui qui l'a fait.
    realisation: t.realisation && typeof t.realisation === "object" ? {
      texte: String(t.realisation.texte || "").trim(),
      le: t.realisation.le || null,
      par: t.realisation.par || null,
    } : null,
    // Pieces qui VALIDENT le livrable : le compte rendu dit ce qui a ete fait,
    // la piece le demontre.
    preuves: Array.isArray(t.preuves)
      ? t.preuves.filter((x) => x && x.documentId).map((x) => ({ documentId: x.documentId, label: String(x.label || "").trim() }))
      : [],
    // Responsabilites rattachees a des PERSONNES, pas a des chaines : on ne
    // convoque pas « le directeur ». Les chaines heritees restent en place et
    // sont signalees comme non resolues.
    ownerId: t.ownerId || null, accountableId: t.accountableId || null,
    consultedIds: Array.isArray(t.consultedIds) ? t.consultedIds.filter(Boolean) : [],
    informedIds: Array.isArray(t.informedIds) ? t.informedIds.filter(Boolean) : [],
    // Clef d'origine dans le modèle type. Permet de réappliquer les paramètres réseau en
    // retrouvant la bonne tâche même si son titre ou sa date ont changé entre-temps.
    tplKey: t.tplKey || null,
    // « A lieu APRES la rentree » (bilan d'ouverture, revue de fin de projet) : une
    // intention du modele, pas une lecture de la date. Un plan rebase qui deborde la
    // rentree a des offsets negatifs sans que ces taches soient pour autant posterieures.
    // `null` = inconnu (plan genere avant ce champ) -> la chaine retombe sur l'offset.
    afterOpening: t.afterOpening == null ? null : !!t.afterOpening,
...
export function updateOpeningTask(oid, tid, patch = {}) {
  ...
  for (const f of ["consultedIds", "informedIds"]) {
    if (f in patch) t[f] = Array.isArray(patch[f]) ? patch[f].filter(Boolean) : [];
  }
  if ("realisation" in patch) {
    t.realisation = patch.realisation && typeof patch.realisation === "object" ? {
      texte: String(patch.realisation.texte || "").trim(),
      le: patch.realisation.le || null,
      par: patch.realisation.par || null,
    } : null;
  }
  if ("preuves" in patch) {
    t.preuves = Array.isArray(patch.preuves)
      ? patch.preuves.filter((x) => x && x.documentId).map((x) => ({ documentId: x.documentId, label: String(x.label || "").trim() }))
      : [];
  }
  // Horodatage de la réalisation. On ne l'écrase pas si la tâche était déjà faite (un
  // simple passage done → doing → done ne doit pas réécrire l'histoire), et on l'effface
  // si elle redevient à faire, sinon la chaîne garderait un retard purgé.
  if ("status" in patch) {
    if (t.status === "done" && !t.doneAt) t.doneAt = new Date().toISOString().slice(0, 10);
...
// Toute cette famille modifie un objet IMBRIQUÉ dans une ouverture (une tâche, son
// livrable, son étape, son commentaire). La référence de la LIGNE `openings` ne bouge
// donc pas, et le persister — qui compare des références — n'écrit rien : la mémoire
// porte la modification, la base non, et on ne s'en aperçoit qu'au redémarrage. Huit
// fonctions l'avaient oublié. Une porte de sortie unique rend l'oubli impossible.
function ecrireOuverture(db, o) {
  if (o) touch("openings", o);
  write(db);
}
export function addTaskOutput(oid, tid, data = {}) {
  const db = read();
  const [o, t] = findTask(db, oid, tid);
  if (!t) return null;
  t.outputs = t.outputs || [];
  const out = normOutput(data);
  t.outputs.push(out);
  ecrireOuverture(db, o);
  return out;
}
...
export const COMMITTEE_SCOPES = ["opening", "campus", "network", "projet"];
...
function normSession(s = {}) {
  return {
    ...
    // Une convocation sans heure ni lieu n'est pas une convocation.
    time: String(s.time || "").trim(), place: String(s.place || "").trim(), link: String(s.link || "").trim(),
    // Horodatage des envois, PAR NATURE d'envoi. Un rappel automatique qui repart chaque
    // jour est pire que pas de rappel du tout : les destinataires filtrent la boîte et
    // la convocation suivante se perd avec. On n'envoie chaque chose qu'une fois.
    sent: (s.sent && typeof s.sent === "object") ? { ...s.sent } : {},
    ...
    // L'ordre du jour se remplit depuis le plan du projet TANT QUE personne n'y
    // a touché. À la première édition humaine, ce drapeau tombe et le
    // remplissage automatique se retire de cette séance, définitivement : un
    // outil qui réécrit par-dessus ce qu'on vient d'ajouter se fait désactiver
    // dans la semaine, et à raison.
    agendaAuto: s.agendaAuto !== false,
    agendaAutoAt: s.agendaAutoAt || null,
...
export function addCommittee(data = {}) {
  ...
  const c = {
    ...
    // `cadence` reste le libellé libre affiché sur la convocation. `cadenceRegle`
    // est la règle qui CRÉE les séances : sans elle, « hebdomadaire » décrivait
    // une intention que personne ne tenait passé le deuxième mois.
    cadenceRegle: data.cadenceRegle || null,
    // Diffusion automatique du compte rendu, le lendemain de la séance. Par défaut OUI :
    // sans elle, un compte rendu rédigé reste dans l'app et personne ne l'a lu. Le délai
    // d'un jour laisse le temps de corriger avant qu'il ne parte.
    autoSendMinutes: data.autoSendMinutes !== false,
...
// Horodate un envoi sur une séance. Retourne false si cette nature d'envoi a déjà eu
// lieu : c'est le garde d'idempotence des rappels automatiques.
export function markSessionSent(cid, sid, kind) {
  const db = read();
  const c = (db.committees || []).find((x) => x.id === cid);
  const s = c && (c.sessions || []).find((x) => x.id === sid);
  if (!s) return false;
  s.sent = s.sent || {};
  if (s.sent[kind]) return false;
  s.sent[kind] = new Date().toISOString().slice(0, 10);
  touch("committees", c);
  write(db);
  return true;
}
// Démarque UN envoi. Sert au rattrapage quand l'expédition a échoué après le marquage :
// marquer avant d'envoyer empêche le doublon, démarquer après l'échec permet de réessayer.
export function resetSessionSentKind(cid, sid, kind) {
  const db = read();
  const c = (db.committees || []).find((x) => x.id === cid);
  const s = c && (c.sessions || []).find((x) => x.id === sid);
...
export function updateCommittee(cid, patch = {}) {
  ...
  if ("cadenceRegle" in patch) c.cadenceRegle = patch.cadenceRegle || null;
  if ("autoSendMinutes" in patch) c.autoSendMinutes = !!patch.autoSendMinutes;
...
export function updateSession(cid, sid, patch = {}, { auto = false } = {}) {
  const db = read();
  const c = (db.committees || []).find((x) => x.id === cid);
  if (!c) return null;
  const s = (c.sessions || []).find((x) => x.id === sid);
  if (!s) return null;
  // Une écriture AUTOMATIQUE de l'ordre du jour est refusée dès que quelqu'un
  // l'a repris à la main : c'est la garantie qui rend le remplissage
  // automatique acceptable.
  if (auto && "agendaItems" in patch) {
    if (s.agendaAuto === false) return s;
    s.agendaAutoAt = now();
  } else if ("agendaItems" in patch) {
    s.agendaAuto = false;
  }
  // Changer la date rouvre les envois : une convocation partie pour l'ancienne date ne
  // vaut plus, et sans ce reset le rappel automatique ne repartirait jamais.
  if ("date" in patch && (patch.date || "") !== s.date) { s.date = patch.date || ""; s.sent = {}; }
  ...
  for (const f of ["time", "place", "link"]) if (f in patch) s[f] = String(patch[f] || "").trim();
  // Rendre la main au remplissage automatique est un acte explicite : il écrase
  // une saisie humaine, on ne le déduit jamais d'un autre changement.
  if ("agendaAuto" in patch) s.agendaAuto = patch.agendaAuto !== false;
...
// --- Paramètres d'ouverture (réseau) ---
// Ce que le modèle de rétroplanning ne peut pas deviner et qui vaut pour TOUTES les
// ouvertures : les jalons de la convention de réseau, les seuils internes de validation
// budgétaire, et les délais réels des fournisseurs. Logés dans le singleton `settings`
// plutôt que dans une collection : c'est un réglage réseau, pas une donnée par projet,
// et `settings` est déjà un singleton persisté (lib/db.js SINGLETONS).
//
// Le nombre PRÉSERVE le zéro : `Number(v) || null` transformerait un seuil à 0 € ou un
// délai de 0 semaine en « non renseigné », alors que ce sont des valeurs valides.
const nbOu = (v, d = null) => (v == null || v === "" || isNaN(Number(v)) ? d : Number(v));
const normMilestone = (j) => ({
  id: j.id || id(), title: String(j.title || "").trim(), lot: String(j.lot || "gouv").trim(),
  m: nbOu(j.m, 0), critical: !!j.critical, owner: String(j.owner || "").trim(), notes: String(j.notes || "").trim(),
});
const normThreshold = (s) => ({
  id: s.id || id(), label: String(s.label || "").trim(), minAmount: nbOu(s.minAmount),
  approver: String(s.approver || "").trim(), leadDays: nbOu(s.leadDays, 0),
});
const normLeadTime = (l) => ({
  id: l.id || id(), family: String(l.family || "").trim(), supplier: String(l.supplier || "").trim(),
  leadWeeks: nbOu(l.leadWeeks), amount: nbOu(l.amount), notes: String(l.notes || "").trim(),
});
export function getOpeningSettings() {
  const o = read().settings?.opening || {};
  return { milestones: o.milestones || [], thresholds: o.thresholds || [], leadTimes: o.leadTimes || [] };
}
export function setOpeningSettings(patch = {}) {
  const db = read();
  const cur = db.settings?.opening || {};
  const next = {
    milestones: (patch.milestones ?? cur.milestones ?? []).map(normMilestone).filter((j) => j.title),
    thresholds: (patch.thresholds ?? cur.thresholds ?? []).map(normThreshold).filter((s) => s.label || s.minAmount != null),
    // Une ligne sans famille ne s'appliquerait à aucune commande : on la jette plutôt
    // que de la garder et laisser croire qu'un délai est pris en compte.
    leadTimes: (patch.leadTimes ?? cur.leadTimes ?? []).map(normLeadTime).filter((l) => l.family),
  };
  db.settings = { ...(db.settings || {}), opening: next };
  write(db);
  return next;
}
...
// « stagiaire » = stagiaire de la formation professionnelle. C'est le statut de
// l'apprenti dont le contrat est rompu et qui reste en formation au CFA : il n'est
// PAS sorti. Le confondre avec une sortie fausse les effectifs, l'émargement,
// l'enquête SIFA et les indicateurs de résultats.
export const ENROLLMENT_STATUSES = ["inscrit", "stagiaire", "sorti", "diplome", "rupture", "abandon"];
export const ENROLLMENT_ACTIFS = ["inscrit", "stagiaire"];
...
function normLearner(l = {}) {
  ...
  return {
    ...
    adresse: s(l.adresse),
    // Rubriques exigées par le Cerfa d'alternance. Le NIR n'est délibérément PAS
    // stocké : il est saisi au moment du dépôt et ne persiste jamais (voir
    // lib/cerfa.js). La nationalité est saisie pour le dépôt, pas conservée comme
    // attribut permanent.
    sexe: ["M", "F"].includes(l.sexe) ? l.sexe : "",
    deptNaissance: s(l.deptNaissance),
    nationalite: [1, 2, 3, "1", "2", "3"].includes(l.nationalite) ? Number(l.nationalite) : null,
    regimeSocial: [1, 2, "1", "2"].includes(l.regimeSocial) ? Number(l.regimeSocial) : null,
    situationAvant: l.situationAvant == null || l.situationAvant === "" ? null : Number(l.situationAvant),
    dernierDiplomePrepare: s(l.dernierDiplomePrepare),
    derniereClasse: s(l.derniereClasse),
    diplomeLePlusEleve: s(l.diplomeLePlusEleve),
    sportifHautNiveau: !!l.sportifHautNiveau,
    ...
    // Autorisations parentales : horodatées et à portée explicite. Stocker les
    // coordonnées du représentant légal ne prouve aucun consentement — il faut
    // savoir QUI a autorisé QUOI et QUAND.
    consentements: normConsents(l.consentements),
    notes: s(l.notes),
    createdAt: l.createdAt || now(), updatedAt: now(),
  };
}

// Portées d'autorisation parentale usuelles en établissement de formation.
export const CONSENT_SCOPES = ["droit_image", "sorties", "communication_notes", "acces_portail", "soins_urgence"];
...
function normConsents(list) {
  if (!Array.isArray(list)) return [];
  return list.filter((c) => c && CONSENT_SCOPES.includes(c.scope)).map((c) => ({
    scope: c.scope,
    accorde: !!c.accorde,
    par: String(c.par || "").trim(),           // qui a signé
    le: String(c.le || "").trim(),             // quand
    note: String(c.note || "").trim(),
  }));
}

export function setConsent(lid, { scope, accorde, par, note }) {
  const db = read();
  const l = (db.learners || []).find((x) => x.id === lid);
  if (!l) return null;
  if (!CONSENT_SCOPES.includes(scope)) return { error: "portée d'autorisation inconnue" };
  l.consentements = (l.consentements || []).filter((c) => c.scope !== scope);
  l.consentements.push({ scope, accorde: !!accorde, par: String(par || "").trim(), le: now().slice(0, 10), note: String(note || "").trim() });
  l.updatedAt = now();
  touch("learners", l);
  write(db);
  return l;
}
export function hasConsent(learner, scope) {
  return !!(learner?.consentements || []).find((c) => c.scope === scope && c.accorde);
}
...
export function deleteLearner(lid) {
  ...
  // Les décisions d'acquis portent le nom du décideur et le motif : c'est de la
  // donnée personnelle, elle part avec le dossier.
  db.blockExemptions = (db.blockExemptions || []).filter((e) => e.learnerId !== lid);
  // Positionnement, aménagements et suivis sont nominatifs — et les suivis
  // portent en plus le nom du tuteur en entreprise.
  db.positionnements = (db.positionnements || []).filter((p) => p.learnerId !== lid);
  db.amenagements = (db.amenagements || []).filter((a) => a.learnerId !== lid);
  db.suivis = (db.suivis || []).filter((s) => s.learnerId !== lid);
  db.mobilites = (db.mobilites || []).filter((m) => m.learnerId !== lid);
  // Les INVITATIONS à enquête sont nominatives : elles partent avec le dossier.
  // Les RÉPONSES, elles, ne le sont pas et ne peuvent donc pas être retrouvées
  // — c'est la contrepartie de l'anonymat, et elle est volontaire : supprimer
  // « ses » réponses supposerait de savoir lesquelles sont les siennes.
  db.enqueteInvitations = (db.enqueteInvitations || []).filter((i) => i.destinataireId !== lid);
  db.lmsTraces = (db.lmsTraces || []).filter((t) => t.learnerId !== lid);
  db.remisesReglement = (db.remisesReglement || []).filter((r) => r.learnerId !== lid);
  db.dossiersCpf = (db.dossiersCpf || []).filter((d) => d.learnerId !== lid);
  write(db);
...
function normEnrollment(e = {}) {
  ...
  return {
    ...
    // Passage en stagiaire de la formation professionnelle après rupture, et
    // échéance des 6 mois pendant lesquels le CFA maintient la formation.
    dateStagiaire: s(e.dateStagiaire), finAccompagnement: s(e.finAccompagnement),
...
export function listEnrollments({ learnerId, classId, campusId, statut } = {}) {
  ...
  // `statut` accepte une liste : les apprenants EN FORMATION regroupent les
  // inscrits et les stagiaires post-rupture, qui doivent rester dans les
  // effectifs, sur les feuilles d'émargement et dans les bulletins.
  if (statut) {
    const attendus = Array.isArray(statut) ? statut : [statut];
    items = items.filter((e) => attendus.includes(e.statut));
  }
...
function normCandidate(c = {}) {
  ...
  return {
    ...
    // Provenance d'une donnée importée : sans elle, impossible de répondre à
    // « d'où vient ce prospect et sur quelle base le traitons-nous ? », qui est
    // la première question d'un contrôle sur de la prospection.
    importedAt: s(c.importedAt), importedFrom: s(c.importedFrom), baseLegale: s(c.baseLegale),
...
export function upsertCandidateFromSf(row, contexte = {}) {
  const db = read();
  db.candidates = db.candidates || [];
  const c = db.candidates.find((x) => x.sfId && x.sfId === row.sfId);
  if (!c) {
    const created = normCandidate({ ...row, source: "salesforce",
      importedAt: now(), importedFrom: contexte.org || "", baseLegale: contexte.baseLegale || "" });
    db.candidates.push(created);
    write(db);
    return { action: "created", candidate: created };
  }
  const patch = { nom: row.nom, prenom: row.prenom, email: row.email, telephone: row.telephone,
    formationSouhaitee: row.formationSouhaitee, sfStatut: row.sfStatut, campusId: row.campusId || c.campusId,
    importedAt: now(), importedFrom: contexte.org || c.importedFrom, baseLegale: contexte.baseLegale || c.baseLegale };
...
export function convertCandidate(cid, { classId, schoolYear, interdireCreation = false } = {}) {
  const db = read();
  const c = (db.candidates || []).find((x) => x.id === cid);
  if (!c) return null;
  ...
  if (!learner) {
    // Le plafond de licence est vérifié par l'appelant, mais c'est ICI qu'on
    // sait si un dossier doit vraiment être créé : un rapprochement avec un
    // dossier existant ne consomme aucune place.
    if (interdireCreation) return { quotaAtteint: true };
...
function normContract(c = {}) {
  ...
  return {
    ...
    // Suivi du dépôt auprès de l'OPCO : sans ce champ, la normalisation jetait
    // silencieusement l'état du dossier — et le tableau de bord montrait un
    // contrat « à transmettre » alors qu'il était parti depuis un mois.
    depot: c.depot && typeof c.depot === "object" ? { ...c.depot } : null,
    ...
    // Éligibilité du maître d'apprentissage : l'employeur atteste des conditions
    // de compétence et d'expérience. C'est aussi l'indicateur 13 du référentiel Qualiopi.
    maitreDateNaissance: isoDate(c.maitreDateNaissance), maitreDiplome: s(c.maitreDiplome),
    maitreNiveau: s(c.maitreNiveau), maitreEligibilite: !!c.maitreEligibilite,
    typeContrat: c.typeContrat == null || c.typeContrat === "" ? null : Number(c.typeContrat),
    derogation: c.derogation == null || c.derogation === "" ? null : Number(c.derogation),
    dateConclusion: isoDate(c.dateConclusion),
    dureeHebdoHeures: n(c.dureeHebdoHeures), dureeHebdoMinutes: n(c.dureeHebdoMinutes),
    travauxDangereux: !!c.travauxDangereux,
    avantageNourriture: n(c.avantageNourriture), avantageLogement: n(c.avantageLogement),
    formationDebut: isoDate(c.formationDebut), formationFin: isoDate(c.formationFin),
...
export const RUPTURE_MODE_KEYS = ["essai", "accord", "apprenti", "faute", "exclusion", "diplome"];

export function openRupture(cid, { origine, motif, mode, by }) {
  const db = read();
  const c = (db.contracts || []).find((x) => x.id === cid);
  if (!c) return null;
  ...
  c.rupture = {
    ...
    // Qualification juridique : elle commande les délais et le formalisme.
    // Elle peut être précisée plus tard, mais doit l'être avant la confirmation.
    mode: RUPTURE_MODE_KEYS.includes(mode) ? mode : null,
...
export function advanceRupture(cid, { stage, note, owner, mode, by }) {
  const db = read();
  const c = (db.contracts || []).find((x) => x.id === cid);
  ...
  if (mode !== undefined && mode !== null && RUPTURE_MODE_KEYS.includes(mode)) c.rupture.mode = mode;
  // Confirmer une rupture sans l'avoir qualifiée, c'est ne pas savoir quelle
  // procédure on vient d'appliquer — et donc ne pas pouvoir la défendre.
  if (stage === "confirmee" && !c.rupture.mode) {
    return { error: "qualifier le mode de rupture avant de la confirmer (période d'essai, accord, démission, faute, exclusion, diplôme)" };
  }
  ...
  // Rupture à l'initiative de l'apprenti après la période d'essai : saisine du
  // médiateur, puis 5 jours calendaires avant d'informer l'employeur, puis 7 jours
  // avant que la rupture prenne effet (art. R. 6222-21). Ces délais sont durs :
  // les rater fait tomber la procédure.
  if (stage === "mediation" && !c.rupture.mediateurSaisiLe) {
    const jour = now().slice(0, 10);
    const plus = (n) => { const d = new Date(jour + "T00:00:00Z"); d.setUTCDate(d.getUTCDate() + n); return d.toISOString().slice(0, 10); };
    c.rupture.mediateurSaisiLe = jour;
    c.rupture.informationEmployeurAuPlusTot = plus(5);
    c.rupture.ruptureEffectiveAuPlusTot = plus(12); // 5 + 7
  }
  // La confirmation rompt le contrat et répercute sur l'inscription en cours.
  if (stage === "confirmee") {
    c.status = "rompu";
    const jour = now().slice(0, 10);
    c.rupture.dateRupture = jour;
    // Le CFA doit maintenir l'apprenti en formation pendant 6 mois (art. L. 6231-2),
    // sous statut de STAGIAIRE de la formation professionnelle. L'inscription ne
    // bascule donc PAS en « rupture » avec une date de sortie : l'apprenti reste
    // dans les effectifs, continue d'émarger, et le compteur des 6 mois démarre.
    const fin = new Date(jour + "T00:00:00Z");
    fin.setUTCMonth(fin.getUTCMonth() + 6);
    c.rupture.finAccompagnement = fin.toISOString().slice(0, 10);
    for (const e of db.enrollments || []) {
      if (e.learnerId === c.learnerId && e.statut === "inscrit") {
        e.statut = "stagiaire";
        e.dateStagiaire = jour;
        e.finAccompagnement = c.rupture.finAccompagnement;
...
// La clé « cco » était une coquille pour « ccf » (contrôle en cours de formation)
// et a été persistée en base : on l'accepte encore en lecture et on la normalise.
export const ASSESSMENT_TYPES = ["devoir", "examen", "tp", "oral", "projet", "ccf"];
const normAssessmentType = (v) => (v === "cco" ? "ccf" : v);
...
function normAssessment(a = {}) {
  ...
  return {
    ...
    type: ASSESSMENT_TYPES.includes(normAssessmentType(a.type)) ? normAssessmentType(a.type) : "devoir",
...
// Un seul positionnement par apprenant : le reprendre le remplace. Deux
// positionnements concurrents rendraient indéterminé ce sur quoi le parcours
// a été construit.
export function getPositionnement(learnerId) {
  return (read().positionnements || []).find((p) => p.learnerId === learnerId) || null;
}
export function setPositionnement(data = {}) {
  const db = read();
  db.positionnements = db.positionnements || [];
  if (!data.learnerId) return null;
  const i = db.positionnements.findIndex((p) => p.learnerId === data.learnerId);
  const rec = { ...data, id: i >= 0 ? db.positionnements[i].id : id(), updatedAt: now() };
  if (i >= 0) { db.positionnements[i] = rec; touch("positionnements", rec); } else db.positionnements.push(rec);
  write(db);
  return rec;
}

// Aménagements et suivis : plusieurs par apprenant, chacun daté.
const collectionApprenant = (cle) => ({
  list: ({ learnerId } = {}) => {
    const items = (read()[cle] || []).slice();
    return (learnerId ? items.filter((x) => x.learnerId === learnerId) : items)
      .sort((a, b) => String(b.date || b.updatedAt || "").localeCompare(String(a.date || a.updatedAt || "")));
  },
  add: (data = {}) => {
    const db = read();
    db[cle] = db[cle] || [];
    const rec = { ...data, id: id(), createdAt: now(), updatedAt: now() };
    db[cle].push(rec);
    write(db);
    return rec;
  },
  update: (rid, patch = {}) => {
    const db = read();
    const x = (db[cle] || []).find((y) => y.id === rid);
    if (!x) return null;
    Object.assign(x, patch, { id: x.id, learnerId: x.learnerId, createdAt: x.createdAt, updatedAt: now() });
    touch(cle, x);
    write(db);
    return x;
  },
  remove: (rid) => {
    const db = read();
    const avant = (db[cle] || []).length;
    db[cle] = (db[cle] || []).filter((y) => y.id !== rid);
    const ok = avant !== db[cle].length;
    if (ok) write(db);
    return ok;
  },
});
const _amenagements = collectionApprenant("amenagements");
export const listAmenagements = _amenagements.list;
export const addAmenagement = _amenagements.add;
export const updateAmenagement = _amenagements.update;
export const deleteAmenagement = _amenagements.remove;
const _suivis = collectionApprenant("suivis");
export const listSuivis = _suivis.list;
export const addSuivi = _suivis.add;
export const updateSuivi = _suivis.update;
export const deleteSuivi = _suivis.remove;

// --- Clés d'API ---
// L'empreinte seule est conservée : une clé lisible en base est une clé volée.
export function listApiKeys() { return (read().apiKeys || []).slice().sort((a, b) => String(b.createdAt || "").localeCompare(String(a.createdAt || ""))); }
export function findApiKeyByHash(h) { return (read().apiKeys || []).find((k) => k.hash === h) || null; }
export function addApiKey(data = {}) {
  const db = read();
  db.apiKeys = db.apiKeys || [];
  const rec = { ...data, id: id(), createdAt: now(), revokedAt: null, lastUsedAt: null };
  db.apiKeys.push(rec);
  write(db);
  return rec;
}
export function revokeApiKey(kid) {
  const db = read();
  const k = (db.apiKeys || []).find((x) => x.id === kid);
...
// Trace d'usage, au jour près : elle sert à repérer les clés dormantes, pas à
// profiler. Écrire à chaque appel ferait une écriture disque par requête.
export function touchApiKey(kid, jourISO) {
  const db = read();
  const k = (db.apiKeys || []).find((x) => x.id === kid);
  if (!k || String(k.lastUsedAt || "").slice(0, 10) === jourISO) return false;
  k.lastUsedAt = jourISO;
  touch("apiKeys", k);
  write(db);
  return true;
}

// --- Registres qualité de l'organisme (réclamations, sous-traitance) ---
const collectionCampus = (cle) => ({
  list: ({ campusId } = {}) => {
    const items = (read()[cle] || []).slice();
    return (campusId ? items.filter((x) => x.campusId === campusId) : items)
      .sort((a, b) => String(b.date || b.createdAt || "").localeCompare(String(a.date || a.createdAt || "")));
  },
  get: (rid) => (read()[cle] || []).find((x) => x.id === rid) || null,
  add: (data = {}) => {
    const db = read();
    db[cle] = db[cle] || [];
    const rec = { ...data, id: id(), createdAt: now(), updatedAt: now() };
    db[cle].push(rec);
    write(db);          // ligne nouvelle : la détection par référence suffit
    return rec;
  },
  update: (rid, patch = {}) => {
    const db = read();
    const x = (db[cle] || []).find((y) => y.id === rid);
    if (!x) return null;
    Object.assign(x, patch, { id: x.id, campusId: x.campusId, createdAt: x.createdAt, updatedAt: now() });
    // Collection dynamique : le marquage l'est aussi. Une insertion automatique
    // avait recopié ici le marquage de la fonction voisine, sur une variable
    // hors de portée — les tests l'ont attrapé en levant une ReferenceError.
    touch(cle, x);
    write(db);
    return x;
  },
  remove: (rid) => {
    ...
    db[cle] = (db[cle] || []).filter((y) => y.id !== rid);
    const ok = avant !== db[cle].length;
    if (ok) write(db);   // suppression : détectée par absence au reflet
    return ok;
  },
});
const _reclamations = collectionCampus("reclamations");
export const listReclamations = _reclamations.list;
export const getReclamation = _reclamations.get;
export const addReclamation = _reclamations.add;
export const updateReclamation = _reclamations.update;
export const deleteReclamation = _reclamations.remove;
const _sousTraitants = collectionCampus("sousTraitants");
export const listSousTraitants = _sousTraitants.list;
export const getSousTraitant = _sousTraitants.get;
export const addSousTraitant = _sousTraitants.add;
export const updateSousTraitant = _sousTraitants.update;
export const deleteSousTraitant = _sousTraitants.remove;
const _jurySessions = collectionCampus("jurySessions");
export const listJurySessions = _jurySessions.list;
export const getJurySession = _jurySessions.get;
export const addJurySession = _jurySessions.add;
export const updateJurySession = _jurySessions.update;
export const deleteJurySession = _jurySessions.remove;

// --- Mobilité internationale, taxe d'apprentissage, indicateurs publiés ---
const _mobilites = collectionApprenant("mobilites");
export const listMobilites = _mobilites.list;
export const addMobilite = _mobilites.add;
export const updateMobilite = _mobilites.update;
export const deleteMobilite = _mobilites.remove;
const _taxe = collectionCampus("taxeCampagnes");
export const listTaxeCampagnes = _taxe.list;
export const getTaxeCampagne = _taxe.get;
export const addTaxeCampagne = _taxe.add;
export const updateTaxeCampagne = _taxe.update;
const _indic = collectionCampus("indicateursDeclares");
export const listIndicateursDeclares = _indic.list;
export const addIndicateurDeclare = _indic.add;
export const updateIndicateurDeclare = _indic.update;

// --- Enquêtes (satisfaction — indicateur 30 ; enseignements — indicateur 33) ---
const _enquetes = collectionCampus("enquetes");
export const listEnquetes = _enquetes.list;
export const getEnquete = _enquetes.get;
export const addEnquete = _enquetes.add;
export const updateEnquete = _enquetes.update;
export const deleteEnquete = _enquetes.remove;

// Invitations : QUI a été sollicité, et s'il a répondu. Rien d'autre.
export function listInvitations({ enqueteId, destinataireId } = {}) {
  let items = (read().enqueteInvitations || []).slice();
  if (enqueteId) items = items.filter((i) => i.enqueteId === enqueteId);
  if (destinataireId) items = items.filter((i) => i.destinataireId === destinataireId);
  return items;
}
export function addInvitations(lignes = []) {
  if (!lignes.length) return [];
  const db = read();
  db.enqueteInvitations = db.enqueteInvitations || [];
  const faites = lignes.map((l) => ({ ...l, id: id(), repondu: false, createdAt: now() }));
  db.enqueteInvitations.push(...faites);
  write(db);
  return faites;
}

// LA FONCTION QUI PORTE L'ANONYMAT.
//
// Elle fait DEUX écritures qui ne se connaissent pas : l'invitation passe à
// « répondu », et la réponse est insérée sans aucun identifiant de répondant.
// C'est ce qui permet d'avoir un taux de participation ET des réponses que
// personne ne peut rattacher à quelqu'un — y compris nous, y compris plus tard.
//
// Rompre le lien ICI, à l'écriture, et non à l'affichage, est le seul moyen de
// garantir qu'aucune requête ultérieure, aucun export, aucune restauration de
// sauvegarde ne le rétablisse : la donnée n'existe pas.
export function enregistrerReponse(enqueteId, invitationId, contenu = {}) {
  const db = read();
  const inv = (db.enqueteInvitations || []).find((i) => i.id === invitationId && i.enqueteId === enqueteId);
  if (!inv) return { ok: false, motif: "invitation introuvable" };
  // Une seule réponse par invitation : sans cette garde, un lien partagé
  // permettrait de voter autant de fois qu'on veut.
  if (inv.repondu) return { ok: false, motif: "déjà répondu" };

  inv.repondu = true;
  inv.reponduLe = now();
  touch("enqueteInvitations", inv);

  db.enqueteReponses = db.enqueteReponses || [];
  const rec = {
    id: id(), enqueteId,
    reponses: contenu.reponses || {},
    moduleId: contenu.moduleId || null,
    teacherId: contenu.teacherId || null,
    // Pas de createdAt à la seconde : sur une petite promotion, l'ordre exact
    // des soumissions rapproché de l'horodatage des invitations suffirait à
    // réidentifier. La date seule suffit à dater la campagne.
    date: now().slice(0, 10),
  };
  db.enqueteReponses.push(rec);
  write(db);
  return { ok: true, reponse: rec };
}
export function listReponses({ enqueteId } = {}) {
  const items = (read().enqueteReponses || []).slice();
  return enqueteId ? items.filter((r) => r.enqueteId === enqueteId) : items;
}

// --- Certification (indicateur 16) et actions d'insertion (indicateur 29) ---
const _certifs = collectionCampus("certifications");
export const listCertifications = _certifs.list;
export const getCertification = _certifs.get;
export const addCertification = _certifs.add;
export const updateCertification = _certifs.update;
export const deleteCertification = _certifs.remove;

const _insertion = collectionCampus("actionsInsertion");
export const listActionsInsertion = _insertion.list;
export const getActionInsertion = _insertion.get;
export const addActionInsertion = _insertion.add;
export const updateActionInsertion = _insertion.update;
export const deleteActionInsertion = _insertion.remove;

// --- LMS : ressources pédagogiques et traces de suivi à distance ---
const _ressources = collectionCampus("ressources");
export const listRessources = _ressources.list;
export const getRessource = _ressources.get;
export const addRessource = _ressources.add;
export const updateRessource = _ressources.update;
export const deleteRessource = _ressources.remove;

// Traces : consultations et travaux rendus. On conserve le strict nécessaire à
// la preuve de réalisation — qui, quoi, quand, et si c'est probant. Pas la
// durée de lecture, pas l'adresse IP, pas le navigateur : ce serait un profil
// de navigation, disproportionné au regard de ce qu'il s'agit de démontrer.
export function listLmsTraces({ ressourceId, seanceId, learnerId, from, to } = {}) {
  let items = (read().lmsTraces || []).slice();
  if (ressourceId) items = items.filter((t) => t.ressourceId === ressourceId);
  if (seanceId) items = items.filter((t) => t.seanceId === seanceId);
  if (learnerId) items = items.filter((t) => t.learnerId === learnerId);
  if (from) items = items.filter((t) => t.date >= from);
  if (to) items = items.filter((t) => t.date <= to);
  return items;
}
export function addLmsTrace(data = {}) {
  const db = read();
  db.lmsTraces = db.lmsTraces || [];
  const rec = {
    id: id(), ressourceId: data.ressourceId, learnerId: data.learnerId,
    seanceId: data.seanceId || null, probant: !!data.probant,
    // Un travail rendu porte son contenu ; une consultation ne porte rien.
    rendu: data.probant ? String(data.rendu || "").slice(0, 5000) : null,
    date: now().slice(0, 10), horodatage: now(),
  };
  db.lmsTraces.push(rec);
  write(db);
  return rec;
}

// --- Actes de formation et remise du règlement intérieur ---
const _actes = collectionCampus("actes");
export const listActes = _actes.list;
export const getActe = _actes.get;
export const addActe = _actes.add;
export const updateActe = _actes.update;
export const deleteActe = _actes.remove;

// Remise du règlement : c'est la PREUVE qui est demandée en contrôle, pas le
// document. On enregistre qui, quelle version, quand — et rien d'autre.
export function listRemisesReglement({ campusId, learnerId } = {}) {
  let items = (read().remisesReglement || []).slice();
  if (campusId) items = items.filter((r) => r.campusId === campusId);
  if (learnerId) items = items.filter((r) => r.learnerId === learnerId);
  return items;
}
export function addRemiseReglement(data = {}) {
  const db = read();
  db.remisesReglement = db.remisesReglement || [];
  // Une remise déjà enregistrée pour cette version ne se duplique pas : le
  // compteur de couverture deviendrait faux.
  const deja = (db.remisesReglement || []).find((r) =>
    r.learnerId === data.learnerId && r.version === data.version);
  if (deja) return deja;
  const rec = {
    id: id(), campusId: data.campusId || null, learnerId: data.learnerId,
    version: String(data.version || "").trim(),
    canal: String(data.canal || "").trim(),
    date: now().slice(0, 10), createdAt: now(),
  };
  db.remisesReglement.push(rec);
  write(db);
  return rec;
}

// --- Contrats des intervenants ---
const _ci = collectionCampus("contratsIntervenants");
export const getContratIntervenant = _ci.get;
export const addContratIntervenant = _ci.add;
export const updateContratIntervenant = _ci.update;
export const deleteContratIntervenant = _ci.remove;
export function listContratsIntervenants({ campusId, teacherId } = {}) {
  let items = _ci.list({ campusId });
  if (teacherId) items = items.filter((c) => c.teacherId === teacherId);
  return items;
}

export function listAttestationsVigilance({ contratId } = {}) {
  const items = (read().attestationsVigilance || []).slice();
  return contratId ? items.filter((a) => a.contratId === contratId) : items;
}
export function addAttestationVigilance(data = {}) {
  const db = read();
  db.attestationsVigilance = db.attestationsVigilance || [];
  const rec = { id: id(), contratId: data.contratId, date: data.date || now().slice(0, 10),
    reference: String(data.reference || "").trim(), createdAt: now() };
  db.attestationsVigilance.push(rec);
  write(db);
  return rec;
}

// --- Catalogue de formation et devis ---
const _offres = collectionCampus("offres");
export const listOffres = _offres.list;
export const getOffre = _offres.get;
export const addOffre = _offres.add;
export const updateOffre = _offres.update;
export const deleteOffre = _offres.remove;

const _devis = collectionCampus("devis");
export const listDevis = _devis.list;
export const getDevis = _devis.get;
export const addDevis = _devis.add;
export const updateDevis = _devis.update;
export const deleteDevis = _devis.remove;

// --- Dossiers de financement individuel ---
const _cpf = collectionCampus("dossiersCpf");
export const listDossiersCpf = _cpf.list;
export const getDossierCpf = _cpf.get;
export const addDossierCpf = _cpf.add;
export const updateDossierCpf = _cpf.update;

// --- Répertoire des personnes du projet ---
const _pers = collectionCampus("personnes");
export const listPersonnes = _pers.list;
export const getPersonne = _pers.get;
export const addPersonne = _pers.add;
export const updatePersonne = _pers.update;
export const deletePersonne = _pers.remove;

// --- Journal d'exploitation ---
const _jx = collectionCampus("journalExports");
export const listJournalExports = _jx.list;
export const addJournalExport = _jx.add;

// --- Communication de masse et signatures ---
const _campagnes = collectionCampus("campagnes");
export const listCampagnes = _campagnes.list;
export const getCampagne = _campagnes.get;
export const addCampagne = _campagnes.add;
export const updateCampagne = _campagnes.update;
// Journal des envois : on garde de quoi PROUVER un envoi, pas de quoi
// reconstituer une base de contacts (adresses masquées à la source).
export function listEnvois({ campagneId, destinataireId } = {}) {
  let items = (read().envois || []).slice();
  if (campagneId) items = items.filter((e) => e.campagneId === campagneId);
  if (destinataireId) items = items.filter((e) => e.destinataireId === destinataireId);
  return items.sort((a, b) => String(b.envoyeLe).localeCompare(String(a.envoyeLe)));
}
export function addEnvois(traces = []) {
  if (!traces.length) return 0;
  const db = read();
  db.envois = db.envois || [];
  for (const t of traces) db.envois.push({ ...t, id: id() });
  write(db);
  return traces.length;
}
const _signatures = collectionCampus("signatures");
export const listSignatures = _signatures.list;
export const getSignature = _signatures.get;
export const addSignature = _signatures.add;
export const updateSignature = _signatures.update;


// --- Acquis, dispenses, équivalences et allègements par bloc ---
// Une décision par (apprenant, bloc) : en reprendre une remplace la précédente.
// Empiler deux décisions sur le même bloc rendrait le bulletin indéterminé.
export function listBlockExemptions({ learnerId, blocId } = {}) {
  let items = (read().blockExemptions || []).slice();
  if (learnerId) items = items.filter((x) => x.learnerId === learnerId);
  if (blocId) items = items.filter((x) => x.blocId === blocId);
  return items;
}
export function setBlockExemption(data = {}) {
  const db = read();
  db.blockExemptions = db.blockExemptions || [];
  const x = normalizeExemption(data);
  if (!x.learnerId || !x.blocId) return null;
  const i = db.blockExemptions.findIndex((e) => e.learnerId === x.learnerId && e.blocId === x.blocId);
  const enregistre = { ...x, id: i >= 0 ? db.blockExemptions[i].id : id(), updatedAt: now() };
  if (i >= 0) { db.blockExemptions[i] = enregistre; touch("blockExemptions", enregistre); } else db.blockExemptions.push(enregistre);
  write(db);
  return enregistre;
}
export function deleteBlockExemption(learnerId, blocId) {
  const db = read();
  const avant = (db.blockExemptions || []).length;
  db.blockExemptions = (db.blockExemptions || []).filter((e) => !(e.learnerId === learnerId && e.blocId === blocId));
  const supprime = avant !== db.blockExemptions.length;
  if (supprime) write(db);
  return supprime;
}
...
export function listGrades({ assessmentId, learnerId } = {}) {
  const toutes = read().grades || [];
  // La copie ne sert qu'aux appelants qui reçoivent la liste ENTIÈRE et
  // pourraient la trier en place. Filtrer produit déjà un nouveau tableau :
  // copier 360 000 notes avant de n'en garder 24 était du travail pur perte, à
  // chaque appel.
  if (!assessmentId && !learnerId) return toutes.slice();
  let items = toutes;
...
// ---------- Financements et facturation (ERP Bloc 5) ----------
// Un dossier de financement porte le montant convenu, son mode (voir lib/billing.js)
// et son échéancier. Les factures qui en découlent sont IMMUABLES une fois émises :
// une erreur se corrige par un avoir, jamais par une réécriture — c'est une
// obligation comptable, et c'est aussi ce qui rend la facturation opposable.

export const FUNDING_MODES_KEYS = ["npec", "heures", "forfait"];

function normFunding(f = {}) {
  const s = (v) => String(v || "").trim();
  const n = (v) => (v === "" || v == null ? null : Number(v));
  return {
    id: f.id || id(), campusId: f.campusId || null,
    learnerId: f.learnerId || null, contractId: f.contractId || null,
    financeur: s(f.financeur), typeFinanceur: s(f.typeFinanceur) || "opco",
    numeroDossier: s(f.numeroDossier),
    mode: FUNDING_MODES_KEYS.includes(f.mode) ? f.mode : "npec",
    montant: n(f.montant), prixHoraire: n(f.prixHoraire),
    dateDebut: isoDate(f.dateDebut), dateFin: isoDate(f.dateFin),
    cadence: ["mensuelle", "trimestrielle", "annuelle"].includes(f.cadence) ? f.cadence : "mensuelle",
    echeances: Array.isArray(f.echeances) ? f.echeances : [],
    // Date d'arrêt (rupture) : déclenche la régularisation au prorata des jours
    // réellement exécutés, sans toucher aux factures déjà émises.
    arret: isoDate(f.arret), motifArret: s(f.motifArret),
    notes: s(f.notes),
    createdAt: f.createdAt || now(), updatedAt: now(),
  };
}

export function listFundings({ campusId, learnerId, contractId } = {}) {
  let items = (read().fundings || []).slice();
  if (campusId) items = items.filter((f) => f.campusId === campusId);
  if (learnerId) items = items.filter((f) => f.learnerId === learnerId);
  if (contractId) items = items.filter((f) => f.contractId === contractId);
  return items.sort((a, b) => (b.dateDebut || "").localeCompare(a.dateDebut || ""));
}
export function getFunding(fid) { return (read().fundings || []).find((f) => f.id === fid) || null; }
export function addFunding(data = {}) {
  const db = read();
  db.fundings = db.fundings || [];
  const f = normFunding(data);
  db.fundings.push(f);
  write(db);
  return f;
}
export function updateFunding(fid, patch = {}) {
  const db = read();
  const f = (db.fundings || []).find((x) => x.id === fid);
  if (!f) return null;
  Object.assign(f, normFunding({ ...f, ...patch, id: f.id, createdAt: f.createdAt }));
  touch("fundings", f);
  write(db);
  return f;
}
export function deleteFunding(fid) {
  const db = read();
  // Un dossier ayant déjà produit des factures ne se supprime pas : les pièces
  // comptables doivent rester rattachables à leur origine.
  if ((db.invoices || []).some((i) => i.fundingId === fid && i.status !== "brouillon")) {
    return { error: "des factures ont été émises sur ce dossier — il ne peut plus être supprimé" };
  }
  db.fundings = (db.fundings || []).filter((f) => f.id !== fid);
  db.invoices = (db.invoices || []).filter((i) => i.fundingId !== fid);
  write(db);
  return { ok: true };
}

// --- Factures ---
// Numérotation : séquence continue par campus, par année et par type de pièce
// (FA facture, AV avoir), SANS TROU. Une facture annulée GARDE son numéro et se
// corrige par un avoir : c'est une obligation comptable, et c'est ce qui rend la
// séquence vérifiable.
//
// On lit le plus grand numéro déjà attribué plutôt que de COMPTER les pièces :
// compter reproduirait un numéro déjà pris dès qu'une pièce est supprimée.
const NUMERO_RE = /^(FA|AV)(\d{4})-(\d+)$/;
export function nextInvoiceSeq(campusId, year, type = "FA") {
  let max = 0;
  for (const i of read().invoices || []) {
    if (i.campusId !== campusId) continue;
    const m = NUMERO_RE.exec(String(i.numero || ""));
    if (!m || m[1] !== type || m[2] !== String(year)) continue;
    max = Math.max(max, Number(m[3]) || 0);
  }
  return max + 1;
}

export function listInvoices({ campusId, fundingId, learnerId, status } = {}) {
  let items = (read().invoices || []).slice();
  if (campusId) items = items.filter((i) => i.campusId === campusId);
  if (fundingId) items = items.filter((i) => i.fundingId === fundingId);
  if (learnerId) items = items.filter((i) => i.learnerId === learnerId);
  if (status) items = items.filter((i) => i.status === status);
  return items.sort((a, b) => (b.date || "").localeCompare(a.date || ""));
}
export function getInvoice(iid) { return (read().invoices || []).find((i) => i.id === iid) || null; }

export function addInvoice(data = {}) {
  const db = read();
  db.invoices = db.invoices || [];
  const inv = {
    id: id(), campusId: data.campusId || null, fundingId: data.fundingId || null,
    learnerId: data.learnerId || null,
    numero: String(data.numero || "").trim(),
    date: isoDate(data.date) || now().slice(0, 10),
    periodeDebut: isoDate(data.periodeDebut), periodeFin: isoDate(data.periodeFin),
    destinataire: String(data.destinataire || "").trim(),
    lignes: Array.isArray(data.lignes) ? data.lignes : [],
    totalHT: Number(data.totalHT) || 0, totalTVA: Number(data.totalTVA) || 0, totalTTC: Number(data.totalTTC) || 0,
    mentionExoneration: String(data.mentionExoneration || "").trim(),
    status: "brouillon",
    avoirDe: data.avoirDe || null,   // renseigné si cette pièce est un avoir
    createdAt: now(), emiseAt: null, emisePar: "",
  };
...
// Émission : la facture devient IMMUABLE. C'est le point de non-retour.
export function issueInvoice(iid, by) {
  const db = read();
  const inv = (db.invoices || []).find((i) => i.id === iid);
  if (!inv) return null;
  if (inv.status !== "brouillon") return { error: "facture déjà émise — la corriger par un avoir" };
  if (!inv.lignes.length) return { error: "facture sans ligne" };
  const annee = (inv.date || now()).slice(0, 4);
  const type = inv.avoirDe ? "AV" : "FA";
  let max = 0;
  for (const i of db.invoices || []) {
    if (i.campusId !== inv.campusId) continue;
    const m = NUMERO_RE.exec(String(i.numero || ""));
    if (m && m[1] === type && m[2] === annee) max = Math.max(max, Number(m[3]) || 0);
  }
  inv.numero = inv.numero || `${type}${annee}-${String(max + 1).padStart(5, "0")}`;
  inv.status = "emise";
  inv.emiseAt = now();
  inv.emisePar = String(by || "");
  touch("invoices", inv);
  write(db);
  return inv;
}

// Une facture émise ne se modifie ni ne se supprime : on émet un AVOIR qui la
// contrepasse, et les deux pièces restent au dossier.
export function creditInvoice(iid, { motif, by }) {
  const db = read();
  const src = (db.invoices || []).find((i) => i.id === iid);
  if (!src) return null;
  // Une facture PAYÉE se contrepasse aussi — c'est même le cas le plus courant :
  // l'erreur se découvre après l'encaissement, et l'avoir fonde le remboursement.
  // Seul un brouillon ne se contrepasse pas : il se corrige ou se supprime.
  if (!["emise", "payee"].includes(src.status)) {
    return { error: "seule une facture émise ou payée peut être contrepassée — un brouillon se corrige directement" };
  }
  if ((db.invoices || []).some((i) => i.avoirDe === iid)) return { error: "un avoir existe déjà pour cette facture" };
  if (!String(motif || "").trim()) return { error: "motif de l'avoir obligatoire" };
  db.invoices = db.invoices || [];
  const avoir = {
    ...src, id: id(), numero: "", avoirDe: src.id, status: "brouillon",
    date: now().slice(0, 10), createdAt: now(), emiseAt: null, emisePar: "",
    motif: String(motif).trim(),
    lignes: (src.lignes || []).map((l) => ({ ...l, montant: -Number(l.montant) || 0 })),
    totalHT: -src.totalHT, totalTVA: -src.totalTVA, totalTTC: -src.totalTTC,
  };
  db.invoices.push(avoir);
  src.status = "annulee";
  src.annuleePar = String(by || "");
  src.annuleeAt = now();
  touch("invoices", src);
  write(db);
  return avoir;
...
// --- Règlements ---
export function listPayments({ invoiceId, campusId } = {}) {
  let items = (read().payments || []).slice();
  if (invoiceId) items = items.filter((p) => p.invoiceId === invoiceId);
  if (campusId) items = items.filter((p) => p.campusId === campusId);
  return items.sort((a, b) => (b.date || "").localeCompare(a.date || ""));
}
export function addPayment(data = {}) {
  ...
  if (inv.status === "brouillon") return { error: "facture non émise — elle ne peut pas être réglée" };
  db.payments = db.payments || [];
  const p = {
    id: id(), invoiceId: inv.id, campusId: inv.campusId,
    montant: Number(data.montant) || 0, date: isoDate(data.date) || now().slice(0, 10),
    moyen: String(data.moyen || "").trim(), reference: String(data.reference || "").trim(),
    createdAt: now(),
  };
  db.payments.push(p);
  // Soldée : le cumul des règlements atteint le montant de la facture.
  const cumul = db.payments.filter((x) => x.invoiceId === inv.id).reduce((s, x) => s + x.montant, 0);
  if (Math.abs(cumul - inv.totalTTC) < 0.01 || cumul > inv.totalTTC) inv.status = "payee";
  touch("invoices", inv);
  write(db);
  return p;
}
...
// --- Conduite de projet ---
// Projets, tâches, modèles et journal des replanifications. Aucune de ces
// collections ne recouvre les ouvertures de campus ni les plans d'action : ce
// sont des registres distincts, et les mélanger ferait perdre à chacun ce qui
// le rend utilisable.
const _projets = collectionCampus("projets");
export const getProjet = _projets.get;
export const addProjet = _projets.add;
export const updateProjet = _projets.update;
export function listProjets({ campusId, archive = false } = {}) {
  const items = _projets.list({ campusId });
  return archive ? items : items.filter((p) => !p.archive);
}

export function listProjetTaches({ projetId } = {}) {
  const items = (read().projetTaches || []).slice();
  return (projetId ? items.filter((t) => t.projetId === projetId) : items)
    .sort((a, b) => (a.ordre || 0) - (b.ordre || 0) || String(a.createdAt || "").localeCompare(String(b.createdAt || "")));
}
export const getProjetTache = (tid) => (read().projetTaches || []).find((t) => t.id === tid) || null;

export function addProjetTache(data = {}) {
  const db = read();
  db.projetTaches = db.projetTaches || [];
  const freres = db.projetTaches.filter((t) => t.projetId === data.projetId);
  const rec = {
    ...data, id: id(),
    // Rang d'affichage : une tâche créée sans position se range à la fin, et
    // non au hasard de l'ordre d'insertion en base.
    ordre: data.ordre != null ? Number(data.ordre) : (freres.reduce((m, t) => Math.max(m, t.ordre || 0), 0) + 10),
    createdAt: now(), updatedAt: now(),
  };
  db.projetTaches.push(rec);
  write(db);
  return rec;
}

export function updateProjetTache(tid, patch = {}) {
  const db = read();
  const t = (db.projetTaches || []).find((x) => x.id === tid);
  if (!t) return null;
  Object.assign(t, patch, { id: t.id, projetId: t.projetId, createdAt: t.createdAt, updatedAt: now() });
  touch("projetTaches", t);
  write(db);
  return t;
}

// SUPPRIMER UNE TÂCHE, C'EST AUSSI NETTOYER CE QUI POINTAIT DESSUS. Un lien
// orphelin ou un enfant rattaché au vide rend le plan non calculable : le
// moteur refuserait alors d'afficher la moindre date, pour une tâche qu'on
// vient sciemment de retirer. Les liens sont donc retirés et les enfants
// remontés d'un niveau, et l'appelant est informé de ce qui a bougé.
export function deleteProjetTache(tid) {
  const db = read();
  const t = (db.projetTaches || []).find((x) => x.id === tid);
  if (!t) return null;
  const liensRetires = [];
  const enfantsRemontes = [];
  for (const autre of db.projetTaches) {
    if (autre.id === tid || autre.projetId !== t.projetId) continue;
    const avant = (autre.liens || []).length;
    if (avant) {
      autre.liens = autre.liens.filter((l) => l.deId !== tid);
      if (autre.liens.length !== avant) { liensRetires.push(autre.titre || autre.id); touch("projetTaches", autre); }
    }
    if (autre.parentId === tid) {
      autre.parentId = t.parentId || null;
      enfantsRemontes.push(autre.titre || autre.id);
      touch("projetTaches", autre);
    }
  }
  db.projetTaches = db.projetTaches.filter((x) => x.id !== tid);
  write(db);
  return { supprimee: t.titre || tid, liensRetires, enfantsRemontes };
}

export function deleteProjet(pid) {
  const db = read();
  const avant = (db.projets || []).length;
  db.projets = (db.projets || []).filter((p) => p.id !== pid);
  if (avant === db.projets.length) return false;
  db.projetTaches = (db.projetTaches || []).filter((t) => t.projetId !== pid);
  db.projetJournal = (db.projetJournal || []).filter((j) => j.projetId !== pid);
  write(db);
  return true;
}

// Journal : la mémoire des replanifications. Une date qui bouge sans trace, ce
// sont trois versions du planning en circulation et personne pour dire laquelle
// fait foi.
export function listProjetJournal({ projetId, max = 200 } = {}) {
  const items = (read().projetJournal || []).filter((j) => !projetId || j.projetId === projetId);
  return items.sort((a, b) => String(b.at || "").localeCompare(String(a.at || ""))).slice(0, max);
}
export function addProjetJournal(data = {}) {
  const db = read();
  db.projetJournal = db.projetJournal || [];
  const rec = { ...data, id: id(), at: now() };
  db.projetJournal.push(rec);
  write(db);
  return rec;
}

export function listProjetModeles() {
  return (read().projetModeles || []).slice().sort((a, b) => String(a.nom || "").localeCompare(String(b.nom || "")));
}
export const getProjetModele = (mid) => (read().projetModeles || []).find((m) => m.id === mid) || null;
export function addProjetModele(data = {}) {
  const db = read();
  db.projetModeles = db.projetModeles || [];
  const rec = { ...data, id: id(), createdAt: now(), updatedAt: now() };
  db.projetModeles.push(rec);
  write(db);
  return rec;
}
export function updateProjetModele(mid, patch = {}) {
  const db = read();
  const m = (db.projetModeles || []).find((x) => x.id === mid);
  if (!m) return null;
  Object.assign(m, patch, { id: m.id, createdAt: m.createdAt, updatedAt: now() });
  touch("projetModeles", m);
  write(db);
  return m;
}
export function deleteProjetModele(mid) {
  const db = read();
  const avant = (db.projetModeles || []).length;
  db.projetModeles = (db.projetModeles || []).filter((m) => m.id !== mid);
  const ok = avant !== db.projetModeles.length;
  if (ok) write(db);
  return ok;
}
