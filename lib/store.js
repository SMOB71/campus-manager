...
// Rappel : une ligne modifiée SUR PLACE (Object.assign, affectation de champ,
// ou accès calculé `row[k] = …`) garde sa référence. Rien ne permet alors de
// détecter la modification par comparaison de pointeurs : la fonction DOIT
// appeler `touch()`. Le test « couverture de touch() » relit ce fichier et
// échoue si une fonction l'oublie — y compris les accès calculés, qui m'avaient
// échappé au premier passage (updateCampus, updateIncident, updateDecision…).

const usePg = () => pg.configured();
let _reflet = null;          // collection -> Map(id -> référence de la ligne)
let _marquees = new Map();   // collection -> Map(id -> ligne) marquées
let _chaine = Promise.resolve();
let _desynchronise = null;

// Marque une ligne modifiée SUR PLACE. Sans cet appel, la modification serait
// invisible à la détection par référence — donc perdue en base, et personne ne
// s'en apercevrait avant un redémarrage.
//
// Pourquoi ne pas comparer les valeurs à la place : mesuré sur un réseau de 30
// campus, la comparaison par valeur portait sur 45 000 lignes et coûtait
// 326 à 432 ms À CHAQUE ÉCRITURE. Un marquage explicite ramène ce coût à zéro,
// au prix d'une discipline — que le test « couverture de touch() » impose.
export function touch(collection, row) {
  if (!row?.id || !usePg()) return row;
  if (!_marquees.has(collection)) _marquees.set(collection, new Map());
  // On retient la LIGNE, pas seulement son identifiant. Aller la rechercher dans
  // le reflet au moment d'écrire renvoyait l'ANCIENNE version quand la ligne
  // avait été remplacée par index (`db.x[i] = nouvelle`) : la nouvelle valeur
  // n'était jamais persistée. Le test de cohérence l'a attrapé.
  _marquees.get(collection).set(row.id, row);
  return row;
}

// Le reflet retient, par collection : la RÉFÉRENCE du tableau, sa LONGUEUR, et
// les lignes par identifiant. Les deux premières permettent de conclure qu'une
// collection n'a pas bougé SANS la parcourir — c'est ce qui rend une écriture
// proportionnelle aux changements et non au volume total.
function prendreReflet(db) {
  const r = {};
  for (const c of pg.COLLECTION_NAMES) {
    const arr = db[c] || [];
    const ids = new Map();
    for (const row of arr) if (row?.id) ids.set(row.id, row);
    r[c] = { arr, len: arr.length, ids };
  }
  for (const s of pg.SINGLETONS) r[s] = canonique(db[s] ?? {});
  return r;
}

// Différence entre l'état vivant et le reflet.
//
// CE QUI REND UNE ÉCRITURE BON MARCHÉ — parcourir toutes les collections coûtait
// 406 000 comparaisons par écriture sur un réseau de 30 campus, soit 170 à
// 200 ms mesurés. Une écriture ordinaire ne touche QU'UNE collection : les
// autres se démontrent inchangées sans être parcourues, en comparant la
// référence du tableau et sa longueur.
//
//   • référence identique, longueur identique, aucune ligne marquée
//       → rien n'a bougé, on ne parcourt PAS ;
//   • référence identique, longueur plus grande (push)
//       → les nouvelles lignes sont en fin de tableau, on ne lit que celles-là ;
//   • référence différente (filter) ou longueur plus petite (splice)
//       → parcours complet, mais de CETTE collection seulement.
//
// Une affectation par index (`db.x[i] = autreObjet`) échapperait à ce raccourci.
// Le magasin n'en contient aucune, et le test de cohérence l'attraperait.
function diff(db) {
  const maj = [], sup = [], singles = [];
  for (const c of pg.COLLECTION_NAMES) {
    const r = _reflet[c] || (_reflet[c] = { arr: [], len: 0, ids: new Map() });
    const cur = db[c] || [];
    const marquees = _marquees.get(c);
    const memeTableau = cur === r.arr;

    if (memeTableau && cur.length === r.len) {
      // Collection inchangée en structure : seules les lignes explicitement
      // marquées peuvent avoir changé.
      if (marquees) for (const row of marquees.values()) maj.push([c, row]);
      continue;
    }
    if (memeTableau && cur.length > r.len) {
    ...
    // Tableau remplacé ou raccourci : il faut le parcourir pour savoir ce qui
    // reste et ce qui a disparu.
    const vus = new Set();
    for (const row of cur) {
      if (!row?.id) continue;
      vus.add(row.id);
      if (r.ids.get(row.id) !== row || marquees?.has(row.id)) maj.push([c, row]);
    }
    for (const id of r.ids.keys()) if (!vus.has(id)) sup.push([c, id]);
  }
  for (const s of pg.SINGLETONS) {
    const v = canonique(db[s] ?? {});
    if (_reflet[s] !== v) singles.push([s, db[s] ?? {}]);
  }
  return { maj, sup, singles };
}

// Les écritures sont enchaînées, jamais parallélisées : deux mises à jour de la
// MÊME ligne doivent atterrir dans l'ordre où elles ont été demandées, et un
// pool de connexions ne le garantit pas.
function persister(db) {
  const { maj, sup, singles } = diff(db);
  // Diagnostic : une écriture ordinaire touche quelques lignes. Si ce compteur
  // s'envole, la détection est cassée et on réécrit la base entière.
  if (process.env.DB_TRACE && maj.length + sup.length > 50) {
    const parCollection = {};
    for (const [c] of maj) parCollection[c] = (parCollection[c] || 0) + 1;
    console.log(new Date().toISOString(), "DB_TRACE écriture massive :", maj.length, "maj", sup.length, "sup", JSON.stringify(parCollection));
  }
  _marquees = new Map();
  if (!maj.length && !sup.length && !singles.length) return;
  // Le reflet se met à jour INCRÉMENTALEMENT : le reconstruire entièrement
  // coûtait deux millions d'insertions dans des Map à chaque écriture.
  for (const [c, row] of maj) _reflet[c].ids.set(row.id, row);
  for (const [c, id] of sup) _reflet[c].ids.delete(id);
  // La référence et la longueur doivent suivre, sinon la prochaine écriture
  // croirait la collection encore modifiée et la réécrirait entièrement.
  for (const c of pg.COLLECTION_NAMES) { const arr = db[c] || []; _reflet[c].arr = arr; _reflet[c].len = arr.length; }
  for (const [s, v] of singles) _reflet[s] = canonique(v);

  _chaine = _chaine.then(async () => {
    ...
    // dit fort, et on resynchronise depuis la base plutôt que de servir un état
    // divergent : la mutation est perdue, la cohérence est sauvée.
    _desynchronise = e.message;
    console.error(new Date().toISOString(), "ÉCRITURE POSTGRESQL ÉCHOUÉE :", e.message);
    try { _cache = { ...EMPTY, ...(await pg.loadAll()) }; _reflet = prendreReflet(_cache); _desynchronise = null; }
    catch (e2) { console.error("resynchronisation impossible :", e2.message); }
  });
}

// Sérialisation canonique : PostgreSQL stocke le jsonb sous forme normalisée et
// en réordonne les clés. Comparer deux JSON.stringify bruts signalerait donc une
// divergence sur des valeurs rigoureusement identiques — et masquerait les
// vraies. La comparaison doit être indépendante de l'ordre des clés.
...
// Contrôle de complétude, réservé aux tests : compare l'état en mémoire à la
// base, ligne à ligne. Une modification sur place non marquée apparaît ici.
export async function verifierCoherence() {
  if (!usePg()) return { ok: true, ecarts: [] };
  await flush();
  const enBase = await pg.loadAll();
  const ecarts = [];
  for (const c of pg.COLLECTION_NAMES) {
    const base = new Map((enBase[c] || []).map((r) => [r.id, canonique(r)]));
    for (const row of (_cache?.[c] || [])) {
      if (!row?.id) continue;
...
// À appeler avant de répondre à une mutation HTTP : la réponse ne doit pas
// partir avant que l'écriture soit durable.
export async function flush() {
  await _chaine;
  if (_desynchronise) { const m = _desynchronise; _desynchronise = null; throw new Error(m); }
}

// Chargement initial. Sans DATABASE_URL, le comportement fichier est conservé à
// l'identique — ce qui permet de faire tourner toute la suite de tests dans les
// deux modes et de comparer.
export async function init() {
  if (!usePg()) return { mode: "fichier" };
  await pg.ensureSchema();
  _cache = { ...EMPTY, ...(await pg.loadAll()) };
  _reflet = prendreReflet(_cache);
  _marquees = new Map();
  return { mode: "postgres", collections: pg.COLLECTION_NAMES.length };
}
