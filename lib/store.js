...
// Mutées en place → comparaison par valeur obligatoire.
export const COLLECTIONS_MUTEES_EN_PLACE = new Set([
  "campuses", "learners", "enrollments", "assessments", "contracts", "invoices",
  "candidates", "incidents", "partners", "classes", "periods", "openings",
  "curricula", "rooms", "teachers", "fundings", "si", "decisions", "reviews",
  "events", "recoveries", "arbitrages", "committees", "scenarios", "actions",
  "deliverables", "visits", "kpi", "networkObjectives", "portalAccess",
  "amenagements", "positionnements", "reclamations", "sousTraitants",
  "jurySessions", "apiKeys", "emailPriority", "emailMuted", "briefs",
]);

const usePg = () => pg.configured();
let _reflet = null;          // collection -> Map(id -> référence|valeur sérialisée)
let _chaine = Promise.resolve();
let _desynchronise = null;

const cleReflet = (c, row) => (COLLECTIONS_MUTEES_EN_PLACE.has(c) ? JSON.stringify(row) : row);

function prendreReflet(db) {
  const r = {};
  for (const c of pg.COLLECTION_NAMES) {
    const m = new Map();
    for (const row of db[c] || []) if (row?.id) m.set(row.id, cleReflet(c, row));
    r[c] = m;
  }
  for (const s of pg.SINGLETONS) r[s] = JSON.stringify(db[s] ?? {});
  return r;
}

// Différence entre l'état vivant et le reflet. Renvoie ce qu'il faut écrire.
function diff(db) {
  const maj = [], sup = [], singles = [];
  for (const c of pg.COLLECTION_NAMES) {
    const avant = _reflet[c] || new Map();
    const vus = new Set();
    for (const row of db[c] || []) {
      if (!row?.id) continue;
      vus.add(row.id);
      if (avant.get(row.id) !== cleReflet(c, row)) maj.push([c, row]);
    }
    for (const id of avant.keys()) if (!vus.has(id)) sup.push([c, id]);
  }
  for (const s of pg.SINGLETONS) {
    const v = JSON.stringify(db[s] ?? {});
...
export function setPositionnement(data = {}) {
  ...
  if (i >= 0) db.positionnements[i] = rec; else db.positionnements.push(rec);
  write(db);
  return rec;
}
...
export function setBlockExemption(data = {}) {
  ...
  if (i >= 0) db.blockExemptions[i] = enregistre; else db.blockExemptions.push(enregistre);
  write(db);
  return enregistre;
}
