...
// MODES DE RUPTURE — la qualification juridique commande la procédure applicable :
// délais, formalisme, parties à informer. Un workflow de pilotage qui ne la porte
// pas laisse le CFA appliquer la mauvaise procédure.
export const RUPTURE_MODES = {
  essai: {
    label: "Rupture unilatérale (période d'essai)",
    quand: "Pendant les 45 premiers jours, consécutifs ou non, de formation pratique en entreprise",
    procedure: "Écrit de l'employeur ou de l'apprenti, notifié au CFA et à l'opérateur ayant déposé le contrat. Aucun motif à justifier.",
  },
  accord: {
    label: "Accord des deux parties",
    quand: "À tout moment après la période d'essai",
    procedure: "Écrit signé des deux parties, notifié au CFA et à l'opérateur.",
  },
  apprenti: {
    label: "Démission de l'apprenti",
    quand: "Après la période d'essai",
    procedure: "Saisine préalable du médiateur consulaire, puis 5 jours calendaires minimum avant d'informer l'employeur, puis 7 jours minimum avant que la rupture prenne effet.",
  },
  faute: {
    label: "Faute grave, inaptitude ou force majeure",
    quand: "À l'initiative de l'employeur, après la période d'essai",
    procedure: "Procédure disciplinaire ou constat médical selon le cas ; licenciement notifié.",
  },
  exclusion: {
    label: "Exclusion définitive du CFA",
    quand: "Après décision disciplinaire de l'établissement",
    procedure: "L'employeur peut engager la rupture ; le CFA notifie sa décision.",
  },
  diplome: {
    label: "Obtention du diplôme avant le terme",
    quand: "À l'initiative de l'apprenti ayant obtenu sa certification",
    procedure: "Préavis écrit d'un mois minimum.",
  },
};
...
export function contractAlerts(contract, todayISO, cfaDays = null) {
  ...
  // Un contrat ROMPU n'alerte plus sur ses échéances propres (essai, fin, dépôt)
  // — elles n'ont plus d'objet. Mais l'ACCOMPAGNEMENT POST-RUPTURE, lui, court
  // précisément parce que le contrat est rompu : c'est la seule alerte qui doit
  // survivre à la rupture, et l'oublier ferait sortir l'apprenti des radars.
  if (contract.rupture?.finAccompagnement && contract.rupture.stage === "confirmee") {
    const restant = days(today, contract.rupture.finAccompagnement);
    if (restant >= 0 && restant <= 45) {
      out.push({ type: "accompagnement", severity: restant <= 15 ? "high" : "medium", date: contract.rupture.finAccompagnement,
        label: `Fin de l'accompagnement post-rupture dans ${restant} jour(s) — statuer sur la suite` });
    } else if (restant < 0) {
      out.push({ type: "accompagnement", severity: "high", date: contract.rupture.finAccompagnement,
        label: `Accompagnement post-rupture échu depuis ${-restant} jour(s) — l'inscription doit être clôturée ou l'apprenti replacé` });
    }
  }
  if (["rompu", "termine"].includes(contract.status)) return out;
  ...
  // Délais du médiateur : tant qu'ils courent, la rupture ne peut pas prendre effet.
  if (contract.rupture?.ruptureEffectiveAuPlusTot && !["resolue", "confirmee"].includes(contract.rupture.stage)) {
    if (today < contract.rupture.ruptureEffectiveAuPlusTot) {
      out.push({ type: "mediation", severity: "medium", date: contract.rupture.ruptureEffectiveAuPlusTot,
        label: `Délais de médiation en cours — rupture possible au plus tôt le ${contract.rupture.ruptureEffectiveAuPlusTot}` });
    }
  }
